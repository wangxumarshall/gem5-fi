...
# -*- coding: utf-8 -*-
"""
案 #23 (127.0.0.1-2026-09-11-16:30:10) 指令级代数闭合复算
全部算术在 mod 2^64 下进行（AArch64 通用寄存器语义）。

dmesg 寄存器事实（vmcore-dmesg.txt L2596-2643）：
  pc  = find_busiest_group+0x140/0xb60
  Code: f9400782 f879d814 2a1903e0 8b14003b (f9409377)
  x27 = ffa90aeec68997bf
  x20 = ffa9342267e000ff
  x1  = ffffd6cc5ea996c0
  x25 = 0xa4 (164)
  x23 = 0x400
  FAR = ffa90aeec68998df
...
print("=" * 78)
print("案 #23 代数闭合复算（mod 2^64）")
print("=" * 78)

x27 = 0xffa90aeec68997bf
x20 = 0xffa9342267e000ff
x1  = 0xffffd6cc5ea996c0
x25 = 0xa4
x23 = 0x400
FAR = 0xffa90aeec68998df
...
print("  再前 2a1903e0   = orr  x0, wzr, w25 (w25=0xa4 → x0=164, 与 x0 dump=0xa4 吻合)")
print("  再前 f879d814   = ldr  x20, [x0, x25, sxtw #3]  ← __per_cpu_offset[164] 装载")
print(f"  x25=164 → __per_cpu_offset[164]，即 CPU164 的 percpu 偏移槽（跨槽：本核 179 未读自己的槽）")
print(f"  x20 装载返回 = {h(x20)} —— 即被腐化的 __per_cpu_offset[164] 读出值")

# ---------------------------------------------------------------
print("\n" + "-" * 78)
print("四、x20 = ffa9342267e000ff 位级 16 位段分析（双端 ff 沾染形态）")
print("-" * 78)
s20 = seg16(x20)
print(f"  x20 = {h(x20)}")
print(f"    [63:48] = 0x{s20[0]:04x}   ← 非 canonical 内核标签（真值此段应 0xffff）")
print(f"    [47:32] = 0x{s20[1]:04x}   ← 0x3422：随机数据段")
print(f"    [31:16] = 0x{s20[2]:04x}   ← 0x67e0：随机数据段")
print(f"    [15: 0] = 0x{s20[3]:04x}   ← 0x00ff，低 12 位=0x0ff，页对齐段异常（真值低 12 位应 0x000）")

print("\n  -- 对照：x1 = ffffd6cc5ea996c0（rq 指针，canonical 真值）--")
s1 = seg16(x1)
print(f"  x1 = {h(x1)}")
...
print("\n  关键观察：")
print("  1. x20[63:48]=0xffa9 —— canonical 内核指针此段必为 0xffff；")
print("     ff 仅存于顶字节 [63:56]（bit63..56），次字节为 a9——半标签钉顶。")
print("  2. x20[15:0]=0x00ff —— ff 又出现在底字节 [7:0]：同一 0xffff 标签的")
print("     另一 ff 字节被搬运到寄存器最低字节——『双端 ff 沾染』。")
print("  3. x20[11:0]=0x0ff ≠ 0 —— percpu 偏移真值低 12 位恒为 0（4K 页对齐；")
print("     谱系实测真值低 16 位有 e000/4000/8000 等，故不变式取低 12 位），")
print("     该 0x0ff 是注入位，经加法污染 x27[15:0]。")
print("  4. 中段 6 字节 a9 34 22 67 e0 00 与现场所有寄存器值（x1/x21/x22/x24/")
print("     x26/x9/x15）的全部 8 字节窗口及移位重排逐一比对均不匹配——")
print("     判定为高熵随机注入，非任何可见数据的搬运。")
...
fam = [
    ...
    ("案23 09-11-16:30 x20=ffa9342267e000ff", x20),
...
print("""
  ...
  ——固定坏位无法产生这种段间迁移。
  案23 的双端 ff 形态：标签的两个 ff 字节被同时抛到寄存器两端
  （bit63..56 与 bit7..0 相距 56 位），是『撕裂后远距抛掷』的极端样本。""")
...
print(f"  (3) x25 = 164 → __per_cpu_offset[164]（跨槽：CPU179 读 CPU164 的槽，调度域遍历正常行为）")
print(f"  (4) x23 = 0x400 未见使用异常（致命装载目标寄存器，值未落地）")
print(f"  (5) x20 低 12 位 = 0x0ff ≠ 0：percpu 偏移 4K 页对齐不变式被破坏【实锤】")
print(f"  (6) x20[63:48] = 0xffa9 ≠ 0xffff：canonical 标签错位【实锤】")
print()

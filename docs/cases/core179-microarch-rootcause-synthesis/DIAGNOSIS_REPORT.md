...
5. spurious 事件 ESR 的 ISS[6] 经 ARMv8-A ARM（DDI 0487）解析为 **S1PTW（stage-1 页表走查标志）的低位**，非"Overlay"。70/73 S1PTW=1（走查中取错，对 D3 预期）= 干净 D3 承重群体；3/73 S1PTW=0（无走查时取错，机制上非 PTW 读出失败，最可能为合法同核 TLB-invalidation 时序竞争——主线 commit 42f91093b043 所针对那类）= 可能合法竞争掺混——先前"bit6=Overlay 应 RES0"的读法是对 ISS 布局的误读，已更正。

...
* **高度聚集性【实锤】**：这 78 次异常 100% 集中发生于逻辑 CPU 179 上。相比之下，其余 191 个核心在此期间的错误记录为零。
* **独立的软件上下文【强推】**：这五次致命崩溃分别跨越了互不相关的内核子系统（CFS 负载均衡、块设备回写、kblockd、swapper、用户态 epoll），从而排除了局部软件逻辑缺陷的可能性。
* **硬件 RAS 负证据【实锤】**：在整个 12 天的五份系统日志中，硬件错误日志（APEI/GHES/BERT）仅出现启动注册行，**全程零条硬件错误记录**。这确凿地证明了，该物理缺陷的失效粒度完全低于所有已挂接的架构级 RAS 检测器，成功绕过了系统的监控体系（或相应的数据通路未被 ECC/奇偶校验覆盖）。
...
在分析崩溃现场时，我们发现异常源于 `ldr` 指令加载 `__per_cpu_offset[i]` 静态数组时的数值损坏（该数组为 write-once 属性，启动后永不改变）。通过将寄存器接收到的错误值与内存真值进行逐位对照，我们提取出了**完美的字节通道偏移特征【实锤】**：
...
| 08-14 | `__per_cpu_offset[176]` | `ffffd937172de000` | `d93715ba0000ffff` | **与 `slot[0]` 循环左移 16 位的形状仅差 1 字节【强推】** |
| 08-17 | `__per_cpu_offset[175]` | （转储不完整） | `00ffffa827b20fe0` | **与上述案例同属一族特征【强推】** |
...
* **跨 set 几何裁决**：在 L1D 组相联几何结构下，受损值的数据源（`slot[0]` 位于 set 87）与当前的装载目标（`slot[146]` 位于 set 105）**处于完全不同的 set**。这一证据【实锤】排除了 L1D 阵列 way/列选通错误的可能，将缺陷确切指向了 Fill-Buffer 的合并级。物理诠释为：Fill-Buffer 中最旧的条目（启动时首次访问留下）被以错误的字节通道相位，错误地回放给了跨 set 的另一个装载操作。

### 3.3 缺陷路径 D2：地址呈现通路 (Address Presentation Path)
取证过程中揭露了另一种叠加失效模式。在部分崩溃场景中，受损的寄存器被用作基址向内存发起请求，导致非规范地址解引用。
根据 ARMv8 不变量要求，`FAR_EL1[63:0]` 必须精确等于 MMU 实际试图翻译的地址。我们利用此不变量进行了对照：
...
**取证结论 (D2)【实锤-弱】**：诚实地修正判定，D2 确凿证据在 5 例崩溃中占 2 例（另外 2 例因 D1 产出的坏值 MSB 恰好为 0，导致 D2 的覆盖损坏不可观察）。这 2 例确凿证据指向了从 AGU 计算结果传递到 MMU 翻译前的微小电气周期内，地址呈现路径的最高字节（Byte 7）被某异常通路强制清零。

### 3.4 缺陷路径 D3：硬件页表漫游读取 (PTW Readout Path)
数据集中的 73 次“虚假翻译错误”具备以下确凿特征：
* **静态映射目标【实锤】**：72/73 次异常针对的是常驻内核的静态线性映射地址（`.data` / `slab` / `vmemmap`）。这些页表一旦在启动时建立便数日未变，根本不满足主线中“并发新建映射竞态”的触发前提。
* **单点孤立性【实锤】**：100% 的异常局限于 CPU 179，与竞态模型通常的跨 CPU 随机分布现象完全不相容。
* **ESR 形状异常【实锤】**：70 例的 ESR 为 `0x96000044`，3 例为 `0x96000004` (FSC=TF-L0)。值得注意的是，其 bit 6（Overlay，在 v8.2 中应为 RES0）恒置为 1，底层硬件成因尚未完全定论。

**取证结论 (D3)**：频繁且局限的虚假错误，无可辩驳地指向了硬件页表漫游器（PTW）在发起读取请求时，其数据返回路径发生了瞬态传导错误。

...
3. **RAS 不兜底**：这些结构对端到端 ECC 隐形（§6.2），故测试逃逸将**直接转化为 SDC**——这正是"结构性测试逃逸 → 静默数据错误"链路的真实实例。
...
| 层级 | 形态 | 覆盖对象 | 对应本案例 |
|---|---|---|---|
| 出厂结构测试 | ATPG（stuck-at/transition/small-delay）＋ PEPR 物理感知区域穷尽 | 位级缺陷 + TIC 缺陷 | 对 D1 的字节旋转部分确定性覆盖 |
| 现场结构扫描 | in-field scan（IFS 类：SAF/ArrayBIST/SBAF） | 逃逸的永久缺陷、老化漂移 | 对已出现 SDC 签名的核心优先 at-speed 扫描 fill-buffer 合并级与字节通道相位 |
| 现场功能 SBST | 指针解引用级 SBST + 欠压（Vmin）功能筛选 | 状态/数据依赖的结构化 SDC | 复现 D1 签名链（§5.1）与 §8 #5 Vmin 向量 |

**边界（诚实保留）。** 上述建议的**有效性论证**来自：(i) 本案例位级法证（D1/D3 签名，§3.2/§3.4）；(ii) 两篇参考论文的硅片数据（PEPR 的 14 nm 30,000+ 芯片 TIC 检测、ITC India 2025 的 HPC 算术模块逃逸分级）。**尚未确立**：PEPR 对"字节通道相位重路由 + 陈旧重放"这类**混合缺陷**的实测检出率（其公开数据限于 TIC 缺陷；D1 的状态依赖半需其时序/序列扩展）；以及本案例故障机（鲲鹏 TSV110）上三级防线各自的现场成本-收益曲线。这些需厂商 RTL/DFT 与现场实验闭合，属 §6/§7 所述"保留给供应商"的范围。
---

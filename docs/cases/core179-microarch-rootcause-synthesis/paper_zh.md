...
1. **缺陷的三路径微架构分解（D1/D2/D3）**，从五份 vmcore 与社区记录的 TSV110 缓存几何结构（`docs/cpu/kunpeng.md`；非厂商 datasheet——§2.1）中位级地导出，*假设* SDC 位于加载数据返回路径（D1，**15:58 案例已确立；08-14 印证头部槽位偏好但为 Hamming-6 而非 Hamming-0——见 §3.2**）、AGU→MMU 地址路径（D2，**未证候选，其硅证据被 D1 混淆——见 §3.3**）和页表漫游器读出路径（D3，**证据强**）——并对三种最强的审稿人质疑（巧合、寄存器转储陈旧、合法的 OOO 漫游竞争）预先作出反驳。该分解是*假设层级*，非均匀定位，且这些签名**与**三条具名数据路径**一致**，而非在硅上独立验证的机制：D1 的 15:58 案例逐位精确（Hamming-0 旋转——方向退化：ror1≡rol7——+ 位翻转不可达，并给出明确的 XOR 距离上界——陈旧行回放源为 30 bit、真值为 26 bit，已从 0102 单板原 vmcore 独立复现；08-14 仅达 Hamming-6）；D3 有 73 次静态映射上的 spurious 错；D2 的 FAR-MSB 证据被 D1 混淆（架构地址本身是 D1 坏值），且 TBI1 分析对那两案例不适用（地址非有效内核地址，故 TBI1-off 无所说明）——故 D2 作为候选而非发现。

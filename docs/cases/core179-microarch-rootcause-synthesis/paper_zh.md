...
1. **缺陷的三路径微架构分解（D1/D2/D3）**，从五份 vmcore 与已公开的 TSV110 缓存几何结构中位级地导出，将 SDC 定位到加载数据返回路径（D1）、AGU→MMU 地址路径（D2）和页表漫游器读出路径（D3）——并对三种最强的审稿人质疑（巧合、寄存器转储陈旧、合法的 OOO 漫游竞争）预先作出反驳。与之竞争的 **PRF（物理寄存器堆）假说被排除**：PRF 损坏无法解释 PTW 路径错误（D3），也无法解释 D1 中位级精确的旧数组头重放及其结构化字节通道偏移——这是一种比任何寄存器单元翻转都更高维度的错误。
...
图 1 将 D1/D2/D3 置于乱序内存子系统之中。三个 🔥 锚点即 §4 的故障注入钩子点；被划掉的 PRF 框记录了本研究排除的竞争性假说。
...
=============================================================================================
   ...
   +-------------------------------------------------------------------+------------------+
   | [X  被排除的假说：物理寄存器堆 (PRF)]                                                |
   | PRF 损坏无法解释 PTW 路径错误 (D3)，也无法解释 D1 中位级精确的                       |
   | 旧数组头重放及其结构化字节通道偏移——一种比任何寄存器单元翻转都更高维度的错误。           |
   +-------------------------------------------------------------------+------------------+
                                                                       |
...
=============================================================================================
  ...
  +----------------------------------------------------+                |
    ...
    [ L2 / L3 / 主存 (RAM) ]            +-----------------------------+
                             ...
                             [ 寄存器写回 (Writeback) ]
```
**图 1.** 乱序 CPU 内存子系统，标注了三个定位到的缺陷锚点（D1/D2/D3 = 故障注入钩子点）以及被排除的 PRF 假说。该图局限于加载与地址翻译子系统；前端与纯计算引擎仅作方位参照。
...
我们已证明（§3.2）真值上的任何单字节位翻转都无法产生观测到的损坏值。结构化 `byte_lane_skew` 模式可以。这就是方法论要点：**结构化数据路径故障是故障注入工具箱针对此类缺陷的必要补充**；仅位翻转的注入器（CHAOS/GeFIN/SiliFuzz 的常态）无法触及。
...
一个规范内核地址（`0xffffffc0…`）的 byte7 被置零为 `0xffffc0…`（非规范）——正是 §3.3 的征兆（架构 MSB ≠ 0 在 MMU 处降为 MSB = 0）。**SE 模式无法产生这一现象**（置零地址仍落在物理内存内且不报错）。这确立 D2 *机制*在仿真中可演练；*定量的* D1-vs-D2 谱可分性（H6 的可证伪核心）尚未确立，需要多种子与 FS 推进到一个可恢复/可对比的状态。

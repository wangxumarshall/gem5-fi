...
- **D2 是未证（从已确认-弱降级）。** §3.3 的 D2 论证三重削弱：(1) `FAR_EL1[63:60]` 对翻译错误是 UNKNOWN/RES0（§2.2，ARMv8-A ARM），故高位 nibble 差异非架构证据；(2) 已验证 `untagged_addr(arch_addr)==FAR`（0814/0824 均成立），若 `TCR_EL1.TBI0/TBI1` 开启此为*预期无缺陷*行为——而我们未从转储记录 TBI；(3) 仅 2/5 恐慌。D2 作为候选假设而非发现。解决需 dump `TCR_EL1` 并确认 TBI 关闭。

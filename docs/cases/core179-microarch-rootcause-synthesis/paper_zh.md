...
1. **缺陷的三路径微架构分解（D1/D2/D3）**，从五份 vmcore 与已公开的 TSV110 缓存几何结构中位级地导出，*假设* SDC 位于加载数据返回路径（D1，**15:58 案例已确立；08-14 印证头部槽位偏好但为 Hamming-6 而非 Hamming-0——见 §3.2**）、AGU→MMU 地址路径（D2，**未证候选，其硅证据被 D1 混淆——见 §3.3**）和页表漫游器读出路径（D3，**证据强**）——并对三种最强的审稿人质疑（巧合、寄存器转储陈旧、合法的 OOO 漫游竞争）预先作出反驳。该分解是*假设层级*，非均匀定位，且这些签名**与**三条具名数据路径**一致**，而非在硅上独立验证的机制：D1 的 15:58 案例逐位精确（Hamming-0 旋转 + 位翻转不可达，并给出明确的 30 bit XOR 距离上界，已从 0102 单板原 vmcore 独立复现；08-14 仅达 Hamming-6）；D3 有 73 次静态映射上的 spurious 错；D2 的 FAR-MSB 证据被 D1 混淆（架构地址本身是 D1 坏值），但 TBI1-off 意味软件剥离不解释它——故 D2 作为候选而非发现。
...
- **D1——加载数据返回路径（fill-buffer / replay-merge）。** 一条加载 `__per_cpu_offset[146]` 的指令返回了 `__per_cpu_offset[0]`（数组头部）右移一个字节后的值（15:58 案例，汉明 0；08-14 案例仅达汉明 6，印证头部槽位偏好）。该字节错位与真值的 XOR 距离为 30 bit，故**无法用任何 k<30 的纯位翻转来表达**；要复现它，需要结构化 `byte_lane_skew` 模型（§4.1），该模型端到端地复现了链条：偏移指针 → 非规范虚拟地址 → 页错误 → Kernel Oops（H5，已验证；§5.1）。
...
- **启动 15:58（实锤）**：`x20 = 0x00ffffcc879da2e0` 匹配 `ror1(__per_cpu_offset[0])`（右旋 1 字节）——*唯一*匹配（汉明距离 0；最近的其它候选是 slot[3]，距离 2）。该加载目标槽位是 146。**已从 0102 单板原 vmcore 独立复现**：`crash` 读出 `__per_cpu_offset[0]=0xffffcc879da2e000`（@ `0xffffb378e29e55d0`）与 `__per_cpu_offset[146]=0xffffcc879ed92000`（装载本应返回的真值）；`ror1(slot[0])=0x00ffffcc879da2e0 == x20`（汉明 0），且 `Hamming(x20, slot[146])=26`（确认 x20 被损坏、非真值）。*（先前版本此处写作 `rol1`，系符号笔误——匹配为右旋，与 §4.1 `byte_lane_skew` 注入器的右旋方向一致。）*
...
两次独立启动，两种不同的旋转幅度（15:58 为 1 字节；08-14 按上述歧义为 6-或-2 字节），都命中*数组头部*（两次均为槽 0——08-14 开机的 `__per_cpu_offset[0]=0xffffd93715b7e000` 与 15:58 的 `0xffffcc879da2e000` 因 per-cpu 偏移数组跨开机/KASLR 不同而异值，但两次匹配槽位均为索引 0、即头部）——15:58 为汉明 0，08-14 为汉明 6。我们**不**在此过度声称"随机巧合概率"（先前版本引用 ≈2⁻⁵⁸；我们撤回其为循环论证——fill-buffer 陈旧行回放模型*预测*损坏值来自最旧的 fill-buffer 条目，即启动时首次访问的数组头部，故"命中头部"是模型一致，而非需排除的巧合）。D1 的承重证据是：(i) 15:58 一例的汉明 0 字节旋转匹配本身（可逐位复现；最近的非头部候选距离为 2），以及 (ii) **位翻转不可达性（现已给出明确穷举上界）**。真值 `slot[0]` 到观测值 `x20` 的 XOR 距离为 `slot[0] ^ x20 = 0xff00334b1a3f42e0`，popcount **30**：因此任何纯位翻转模型（不引入字节置换）必须翻转*整整 30 bit* 才能把真值变成该签名，而一次结构化字节置换（ror1）以 *0 bit 翻转* 即达。对单字节位翻转空间（8 字节 × 256 掩码）的穷举搜索无命中（确认单字节、≤8-bit 翻转不足够）；XOR 距离 30 对任何 k < 30 的纯翻转模型闭合了上界。故损坏是一种结构化的字节通道重路由，而非位翻转——这正是 fill-buffer/load-queue 旧表项以错误字节通道相位重放的征兆。

### 3.3 地址路径征兆 D2

在两次崩溃（08-14、08-24）中，架构地址的 MSB ≠ 0（`0xd9…`、`0x55…`），而内核打印的 `FAR` 却 MSB = 0（`0x00…`）。我们必须诚实指出：此 D2 证据**比先前版本声称的弱**，但精确 TBI 分析*部分恢复*它：(1) **`FAR_EL1[63:60]` 对翻译错误是 UNKNOWN/RES0**（§2.2，ARMv8-A ARM DDI 0487 §D13.2.30）——仅 `FAR[51:0]`（至 [55:0]）架构保证，故高位 nibble 差异本身非架构证据。(2) **TBI1（EL1/内核 top-byte 剥离）未被静态启用**——objdump 0102 单板 vmlinux `__cpu_setup` 显示无 `TCR_EL1.TBI1`（bit 38）立即数；`CONFIG_ARM64_TAGGED_ADDR_ABI=y` 只动态启用 TBI0（EL0/用户）按进程。*（保留：此为对 `__cpu_setup` 初始化路径的静态分析；运行时 `mrs tcr_el1` 读取——经内核模块或 crash 的 per-cpu TCR——才是闭合裁决，且未执行。若后续运行时写 TCR_EL1 置 TBI1，该静态结论不会捕捉到。）* 因 0814/0824 的致错地址是内核态（`find_busiest_group`/`bio_add_page` 是内核函数），TBI1-off 意味内核翻译时*不*对这些地址做 top-byte 剥离——故 FAR-MSB 差异**不**是软件 TBI 伪象（相对早先"依赖 TBI"框架的部分恢复）。但 (3) 0814/0824 的 `arch_addr` 高字节 0xd9/0x55 本身是 D1 坏值的非规范高字节（规范内核地址是 0xffff…），非真内核地址——故"arch MSB ≠ FAR MSB"观察被 D1 混淆。**净结论：D2 从"已确认-弱"降级为"未证候选，部分恢复"——地址通路机制在仿真可演练（§5.3），TBI 不解释内核态 FAR-MSB 差异，但硅证据被 D1 混淆（架构地址本身损坏）。** 完全裁决 D2 需一个架构地址是真正规范内核地址而 FAR 显示不同高字节的案例——5 转储中无此案例。
...
### 5.2 为何位翻转注入不充分：一项可证伪的方法论主张（含边界测试）

我们已证明（§3.2）真值上的任何纯位翻转都无法产生观测到的损坏值：对全部 192 个数组槽位做单字节穷举搜索（8 字节 × 256 掩码）无命中，且真值 `slot[0]` 到 15:58 签名的 XOR 距离为 popcount-30——即一个纯位翻转模型需翻*整整 30 bit*（一个大的、结构化的位集合，非"少数 bit"SEU）才能复现，而结构化 `byte_lane_skew` 模式以 0 bit 翻转、仅一次字节置换即达。我们将此从一项案例观察提升为**可证伪的方法论主张，现已给出明确穷举上界**：

> **对于其签名是陈旧值的*字节通道相位位移*（与某数组头部条目的旋转副本汉明距离为 0、与真值的 XOR 距离为 30 bit、无法表为任何 k < 30 的单 bit 或少数 bit 翻转）的缺陷，位翻转故障注入器原则上是不足以复现该签名的；需要*结构性*（字节重路由）的故障模型。**
...
|---|---|---|---|---|
| `byte_lane_skew`（结构性）| 30 | 28 | 28 | 字节旋转（Hamming-0 rol 匹配）|
| `all_zero`（结构性）| 30 | 29 | 29 | 全零交付 |
| `bit_flip`（1-bit）| 30（numBitFlips=30）| 29 | 29 | 1-bit 掩码（如 0x20）|

**诚实发现：** 位翻转注入在 ptrskew 探针上产出*相同检出率*（29/30 = 97%）。故"位翻转不足够"**不是关于检出率**——而是关于**签名形态**：位翻转产 1-bit 差异损坏，非定义 D1 签名的 Hamming-0 字节旋转匹配。§3.2 的穷举测试（slot[0] 上任何单字节位翻转都产生不了观测值）使位翻转与 D1 *签名不兼容*，非*检出率不兼容*。**这精确界定了 §5.2 主张**：位翻转对许多 SDC 是有效模型（且此处产出等效检出率），但它不能复现 core 179 展现的*字节相位位移签名*；*该签名*需结构模型，非 SDC 检出一般所需。

此主张在波普尔意义上可证伪：只要展示一个有界位翻转模型能复现字节相位位移签名，即可将其证伪。我们对 core 179 做了穷举搜索，未能找到这样的反例。该主张的*适用范围*被诚实界定——它适用于字节相位位移这一*签名类*，而非所有 SDC（许多 SDC 确实是单位 SEU，此时位翻转是正确模型，且边界测试显示位翻转产出等效检出率）。但在其范围内，它是一个工具链设计含义：**对字节相位位移签名类，结构性数据路径故障是故障注入工具箱的必要补充**；仅位翻转的注入器（CHAOS/GeFIN/SiliFuzz 的常态）无法*复现此签名形态*，而一项"干净"的位翻转 SDC 研究若遗漏结构性故障，将静默地对该签名类覆盖不足。这是本文可迁移的方法论贡献——独立于推导出它的具体缺陷，且现已对照位翻转做了边界测试。
...
ECC 开：5 个种子全部 0 次 spurious（每次单 bit 翻转被纠正 → 返回合法 PTE）。ECC 关：每种子 1–4 次 spurious（翻转残留 → PTE 变 invalid → 翻译错，重查成功）。**H7 在方向上已验证（ECC 开 5/5 全 0 spurious、ECC 关 5/5 各 1–4 spurious），并附内部效度保留**：**两臂非严格同路径对照，且 ECC 开臂根本未被注入**——`conditionalValidBit` 翻转在 `ptwEcc` 开时于 ECC gate 处*在发生前即被拦截*（见 `CHAOSPTW.cc` 的 ECC gate 提前 return），故 ECC 开的 0-spurious 反映"未发生扰动"而非"翻转发生并被就地纠正"。ECC 关则被扰动，其残留 invalid PTE 触发重查级联改变执行路径（`numHooksCalled`≈15808 vs ECC 开≈17，`simInsts`≈3090）。故 ECC 旋钮是预期的受控变量，但该对照把"ECC 纠正翻转"与"未发生翻转"混为一谈。5/5 的方向稳定性（ECC 开恒 0、ECC 关恒 >0）稳健，但严格同路径对照需一个就地 ECC 纠正模型（翻转发生、被纠正、执行沿同路径推进）或非级联故障模型——未来工作。（数据：`FI_DESIGN_SUPPLEMENT.md` §7，分支 `fi-h6-h7-fs-verify` commit `3287299`；待当前主机用户态构建链重建后在新建 `gem5.opt` 上独立复现——见 §7。）
...
1. **fill-buffer 合并 / 字节通道多路复用器 at-speed 扫描。** D1 签名——`ror1`（右旋 1 字节）汉明 0（15:58，逐位精确）与一次 6-或-2 字节旋转（ror6≡rol2 歧义，§3.2）汉明 6（08-14，最近）——是 fill-buffer 字节通道选择/合并逻辑的直接 DFT 向量；覆盖 load 返回多路复用器的 8 个字节通道相位控制，并复现"跨组旧头部重放"条件。
...
- **H6/H7 状态被精确界定，未夸大或缩小。** SE 模式空结果被静态归因到 ARM MMU 翻译模型（§2.4、§5.3–5.4），而非注入器逻辑。FS 模式运行确认 D2 与 D3 *钩子*在 MMU 开启翻译时触发，并复现片上 D2 征兆（规范→非规范）。**H7 方向已验证（非完全）**（§5.4）：`conditionalValidBit` 模式使 ECC 成为预期的受控变量，5 个 FS 种子（本机再确认：ECC 开 5/5 全 0 spurious、ECC 关 5/5 各 1–4 spurious）显示方向性对照——但 **ECC 开臂根本未被注入**（翻转在发生前即被 gate 拦截），故该对照把"ECC 纠正翻转"与"未发生翻转"混为一谈；完全验证需一个就地纠正模型使两臂同路径。**H6 方向已观测且有体制内多种子支持证据**：fetch-stall 作 Crash 代理的对照**跨 5 种子可分**（仅 D1 5/5 正常推进 vs 仅 D2 5/5 stall，D1+D2 5/5 stall 且 D2 主导，§5.3）。这是给定 gem5 O3 fetch-stall 模型限制下最强体制内受控对照。guest 可见 Crash/SDC 谱（guest Oops 而非仿真器 stall）仍是未来工作——受**非时间限制**（O3 FS ~12万 inst/s）而是 O3 fetch-stall 模型（KVM CPU 产真 fault 但失 O3-LSQ 钩点）。一位问"你验证 H6/H7 了吗？"的审稿人会得到"H7 方向已验证（5 种子 ECC 对照，但 ECC 开臂实际未被注入——内部效度保留）；H6 方向已观测且有 5 种子体制内 Crash 代理可分性；guest 可见谱受 O3 fetch-stall 注入器架构限制，非时间。"

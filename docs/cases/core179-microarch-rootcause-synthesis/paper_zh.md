...
- **启动 15:58（实锤）**：`x20 = 0x00ffffcc879da2e0` 匹配 `rol1(__per_cpu_offset[0])`——*唯一*匹配（汉明距离 0；最近的其它候选是 slot[3]，距离 2）。该加载目标槽位是 146。**已从 0102 单板原 vmcore 独立复现**：`crash` 读出 `__per_cpu_offset[0]=0xffffcc879da2e000`（@ `0xffffb378e29e55d0`）与 `__per_cpu_offset[146]=0xffffcc879ed92000`（装载本应返回的真值）；`rol1(slot[0])=0x00ffffcc879da2e0 == x20`（汉明 0），且 `Hamming(x20, slot[146])=26`（确认 x20 被损坏、非真值）。
- **启动 08-14（强推，非实锤）**：`x20 = 0xd93715ba0000ffff` 的**最近**旋转匹配为 `ror6(__per_cpu_offset[1])`，XOR（汉明）距离为 **6 bit**——非 0。我们更正先前版本声称的"rol6，汉明 0"：对 `slot[1] ^ ror6(slot[1])` 与 `x20` 的独立复算得 popcount 6（这些位跨字节分布：`15b7`→`15ba` 差 1 bit，外加一次未对齐到干净字节边界的旋转）。故 08-14 是【强推】案例（旋转形状 + 头部槽位偏好），而非如 15:58 那样的【实锤】/汉明 0 案例。位级精确的承重证据由 15:58 一例单独承载；08-14 印证*头部槽位偏好*与*旋转形态*，但不印证汉明 0。

两次独立启动，两种不同的旋转幅度（1 与 6 字节），都命中*数组头部*（槽 0/1）——15:58 为汉明 0，08-14 为汉明 6。我们**不**在此过度声称"随机巧合概率"（先前版本引用 ≈2⁻⁵⁸；我们撤回其为循环论证——fill-buffer 陈旧行回放模型*预测*损坏值来自最旧的 fill-buffer 条目，即启动时首次访问的数组头部，故"命中头部"是模型一致，而非需排除的巧合）。D1 的承重证据是：(i) 15:58 一例的汉明 0 字节旋转匹配本身（可逐位复现；最近的非头部候选距离为 2），以及 (ii) **位翻转不可达性（现已给出明确穷举上界）**。真值 `slot[0]` 到观测值 `x20` 的 XOR 距离为 `slot[0] ^ x20 = 0xff00334b1a3f42e0`，popcount **30**：因此任何纯位翻转模型（不引入字节置换）必须翻转*整整 30 bit* 才能把真值变成该签名，而一次结构化字节置换（ror1）以 *0 bit 翻转* 即达。对单字节位翻转空间（8 字节 × 256 掩码）的穷举搜索无命中（确认单字节、≤8-bit 翻转不足够）；XOR 距离 30 对任何 k < 30 的纯翻转模型闭合了上界。故损坏是一种结构化的字节通道重路由，而非位翻转——这正是 fill-buffer/load-queue 旧表项以错误字节通道相位重放的征兆。
...
1. **fill-buffer 合并 / 字节通道多路复用器 at-speed 扫描。** D1 签名——`rol1` 汉明 0（15:58，逐位精确）与 `ror6` 汉明 6（08-14，最近旋转）——是 fill-buffer 字节通道选择/合并逻辑的直接 DFT 向量；覆盖 load 返回多路复用器的 8 个字节通道相位控制，并复现"跨组旧头部重放"条件。

...
1. **fill-buffer merge / byte-lane-mux at-speed scan.** The D1 signature — `rol1` at Hamming-0 (15:58, bit-exact) and `ror6` at Hamming-6 (08-14, nearest rotation) — is a direct DFT vector for the fill-buffer byte-lane selection/merge logic; cover the load-return mux's 8 byte-lane phase controls and reproduce the "cross-set stale-head replay" condition.

...
D1-only: **5/5 seeds normal progression** (simInsts ≈ 400k). D2-only: **5/5 seeds stall** (simInsts ≈ 3k). D1+D2: **5/5 seeds stall (D2 pre-empts D1)**. The fetch-stall-as-Crash-proxy spectrum is **separable across 5 seeds**: D1 → 0% Crash-proxy, D2 → 100% Crash-proxy. This is the strongest within-FS controlled multi-seed contrast we can produce given the gem5 O3 fetch-stall model limitation. **What is NOT established:** a *guest-visible* Crash/SDC classification (the stall is gem5's `outside of physical memory, stopping fetch` simulator behavior, not a guest Oops; and switching to KVM CPU for real translation faults loses the O3-LSQ hook points). Thus H6 is **direction-observed with within-FS multi-seed supporting evidence (5/5 separable on the Crash-proxy), not guest-separability-confirmed**; the guest-visible spectrum is future work bounded by the O3 fetch-stall injector-architecture limitation (needs a non-O3 fault model that still hooks the address path).

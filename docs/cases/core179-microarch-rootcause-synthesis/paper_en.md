...
> **Honesty preamble.** Every quantitative claim in this paper is reproducible from the artifacts in `docs/cases/core179-microarch-rootcause-synthesis/` and the gem5 tree on branch `fix/paper-review-v1-honesty-hardening` (the honesty-hardening branch; the historical working branch was `docs/core179-microarch-rootcause`). *Scope caveat:* the arithmetic (XOR/Hamming/rotation) is reproducible from the cited values; the SE-mode nulls are reproducible from `run_H6.sh`/`run_H7.sh`; the vmcore-derived forensics are re-verifiable by anyone with the 0102 board via `reproduce_d1_forensic.sh` (the 180 GB vmcores are not redistributable); but the **FS-mode 5-seed H6/H7 quantitative tables were produced by documented manual `o3_chaos_fs.py` runs, not a single shipped script** (see §8 for the exact scope). Where a result could not be verified we say so explicitly rather than omit it. Where a single-run number depends on runtime entropy (seed 0 → `std::random_device`), we report the magnitude as stable-across-runs and flag the run-to-run variance rather than presenting one deterministic figure. The fault-injection host is the same machine that exhibits the defect (CPU 179); we report the mitigation (CPU isolation during build/run) and the residual risk.

---

## Abstract

Silent data corruption (SDC) on a single physical core of a production ARM64 server (HiSilicon Kunpeng-920 / TaiShan V110) manifested as recurring kernel panics across five independent boots over twelve days, every event pinned to logical CPU 179. We perform a five-kdump forensic study combining (1) bit-exact register-vs-memory comparison at the crash instant, (2) ARMv8 architectural-invariant reasoning on `FAR_EL1`, and (3) microarchitectural modeling against community-documented TSV110 geometry (from `docs/cpu/kunpeng.md`, a community-maintained summary — not a vendor datasheet; HiSilicon does not publish a public TSV110 RTL/datasheet, so the geometric arguments are *conditional* on this geometry being correct — see §2.1). We identify signatures **consistent with** three specific microarchitectural data paths (Fig. 1; the mechanism is not independently silicon-verified — see §3.5/§7): **D1** — the core-private load data-return path (fill-buffer/replay-merge ≈ L1D readout mux); **D2** — the AGU→MMU address-presentation path; and **D3** — the page-table-walker (PTW) readout path. The decisive new evidence: a load of `__per_cpu_offset[146]` returned a value bit-identical to `__per_cpu_offset[0]` byte-rotated by one byte — a structural byte-lane skew matching the array head among all 192 slots (Hamming distance 0; the rotation direction is degenerate — `ror1(x) ≡ rol7(x)` for a 64-bit word, so the match is identified up to a (ror1, rol7) ambiguity, and the load-bearing fact is the Hamming-0 rotation itself, unreachable by any bit-flip, not its direction; the XOR-distance from the stale-replay source `slot[0]` — the value the load actually returned, per the stale-replay model — to this signature is 30 bits, so no pure k<30-bit flip reproduces it; the XOR-distance from the *truth* `slot[146]` to the signature is 26 bits). This shows the conventional **bit-flip** fault model cannot reproduce the defect; a **structural** (byte-lane-skew) fault model is required.

To close the conjecture-verification loop we extend the CHAOS gem5 fault injector with a *structural* (byte-lane-skew) fault model and reproduce the kernel oops chain end-to-end in simulation (skewed pointer → non-canonical VA → page fault) — **H5, consistency-checked** (the structural model closes the causal chain; it is a mechanical reproduction whose injector rotation operation *is* the D1 signature's defining operation, so it is a consistency/closure check rather than an independent verification that the defect is a rotation — that rests on §3.2 forensics; see §5.2). We further implement address-path (P-D2) and PTW-readout (P-D3) injectors and derive falsifiable hypotheses H6/H7. Initially these returned null results in syscall-emulation (SE) mode; we **statically root-cause the null to the ARM MMU translation model** (`mmu.cc:1226-1227`: SE has `SCTLR.M=0` → `translateMmuOff` → `setPaddr(vaddr)`, identity mapping that bypasses the page-table walker), and we **confirm in full-system (FS) mode that both D2 and D3 hooks fire under MMU-on translation** (D2: address-path injections on canonical kernel VAs made non-canonical; D3: thousands of PTW-descriptor flips producing spurious-translation-fault counts). We additionally discover and fix a C++ member-initialization-order bug (`rng(rng_seed != 0 ? seed : rd())` with `rng` declared before `rd`) that crashed the injectors under the default `seed=0`; this is why prior H6/H7 *SE* runs (which used `seed≠0`) never crashed while FS runs (default seed) did. The **H7 quantitative** result is **directionally consistent** with an internal-validity caveat (and is *not* "robust" evidence about ECC on silicon): a purpose-built `conditionalValidBit` injection mode (single-bit XOR on bit 0 of block descriptors only) makes ECC the intended controlled variable, and 5 FS seeds show ECC-on → 0 spurious faults vs ECC-off → 1–4 spurious per seed — but the ECC-on arm is *not* injected at all (the flip is gated out before it happens), so the contrast is "no perturbation → 0 spurious" vs "perturbation → spurious", which is mechanically expected and establishes only injector self-consistency, not that ECC corrects a landed flip; a non-cascading / in-place-correction fault model would be needed to close internal validity. The H6 D1-vs-D2 *spectrum-separability* result is **direction-observed, not confirmed**: D2-only FS injection halts execution in 3/3 seeds (reproducing the §3.3 non-canonical signature), while D1-only SE injection yields 93% SDC-detectable corruption — but these are measured in *different* translation regimes (D1 in SE, D2 in FS), D1 does not trigger in FS early-boot (its store→load-forward hook is unexercised there), and the D2 "halt" is a simulator fetch-stall rather than a guest-visible Oops. Controlled within-FS separability is bounded by the O3 full-system rate (~25 h) and is future work. We deliver a DFT query list for the silicon vendor and an honest boundary on what simulation can and cannot adjudicate.

---

## 1. Introduction

Modern server CPUs rely on out-of-order speculative execution to hide memory latency. The very structures that enable this — deep load/store queues, physical register files, fill buffers, page-table walkers — are also where transient, sub-cycle timing defects may hide at a granularity *consistent with, but not proof of, sub-architectural-RAS coverage* (RAS-not-probing and firmware-swallow alternatives are not excluded — see §7). When such a defect is **core-private** and **intermittent**, it produces a signature that looks, at the operating-system level, like a stream of inexplicable panics on one logical CPU and zero elsewhere.
...
1. **A three-path microarchitectural decomposition (D1/D2/D3)** of the defect, derived bit-exactly from five vmcores and community-documented TSV110 cache geometry (`docs/cpu/kunpeng.md`; not a vendor datasheet — §2.1), *hypothesizing* the SDC to the load data-return path (D1, **established at the 15:58 case; 08-14 corroborates head-slot preference but at Hamming-6, not Hamming-0 — see §3.2**), the AGU→MMU address path (D2, **unproven candidate whose silicon evidence is confounded by D1 — see §3.3**), and the page-table-walker readout path (D3, **strong evidence**) — with pre-emptive rebuttals of the three strongest reviewer attacks (coincidence, register-dump staleness, legitimate OOO-walk race). The decomposition is a *hypothesis hierarchy*, not a uniform localization, and the signatures are **consistent with** three named data paths rather than independently silicon-verified mechanisms: D1's 15:58 case is bit-exact (Hamming-0 rotation — direction-degenerate: ror1≡rol7 — + bit-flip unreachability with an explicit XOR-distance bound of 30 bits from the stale-replay source, and 26 bits from the truth, independently re-verified from the original 0102-board vmcore; 08-14 reaches only Hamming-6); D3 has 73 spurious-fault events on static mappings (70/73 walk-consistent; 3/73 S1PTW=0 are a possibly-legal TLB-invalidation-race admixture — see §3.4); D2's FAR-MSB evidence is confounded by D1 (the architectural address is itself the D1-corrupted value), and TBI1 analysis is inapplicable to those two cases (the addresses are not valid kernel addresses, so TBI1-off tells us nothing) — so D2 stands as a candidate, not a finding.
2. **A structural fault-injection methodology** — extending CHAOS/gem5 with `byte_lane_skew` / `all_zero` data-path faults, an address-path injector, and a PTW-readout injector — and the **end-to-end reproduction** of the kernel oops chain via the structural (not bit-flip) model (H5 consistency-checked, not independently verified — see §5.1/§5.2).
...
- **D1 — Load data-return path (fill-buffer / replay-merge).** A load of `__per_cpu_offset[146]` returned `__per_cpu_offset[0]` (array head) byte-rotated by one byte (the 15:58 case, Hamming-0; the 08-14 case reaches only Hamming-6 and corroborates the head-slot preference). *Rotation direction is degenerate for a 64-bit word: `ror1(x) ≡ rol7(x)` (1+7=8 bytes = whole word), so 15:58 is identified under a (ror1, rol7) ambiguity — symmetric to the 08-14 (ror6, rol2) ambiguity; the load-bearing fact is the Hamming-0 rotation, not its direction.* This byte displacement has an XOR-distance of 30 bits from the stale-replay source (`slot[0]`, the value the load actually returned per the stale-replay model) — and 26 bits from the *truth* (`slot[146]`) — so it is **not expressible as any pure k<30-bit flip** (or k<26 from the truth); reproducing it required the structural `byte_lane_skew` model (§4.1), which end-to-end reproduces the chain skewed-pointer → non-canonical VA → page fault → Kernel Oops (H5, consistency-checked; §5.1).
- **D2 — Address path (AGU → MMU), candidate hypothesis (unproven — §3.3).** In two crashes the architectural register carries an address with MSB ≠ 0 yet `FAR_EL1` records MSB = 0. This was earlier read as "MSB-zeroing on the address path", but §2.2/§3.3 now show the difference lives in `FAR_EL1[63:60]` (UNKNOWN for translation faults) and that the 0814/0824 `arch_addr` high bytes are themselves D1-corrupted (non-canonical), so TBI analysis is inapplicable to these two cases — so D2 is a *candidate*, not a finding. In gem5 SE mode (`SCTLR.M=0`, identity VA==PA) a zeroed address lands in valid physical memory and faults nowhere (null); in FS mode zeroing a kernel VA (`0xffff…`) yields a non-canonical address (`0xffffc0…`) that raises a translation fault (§5.3) — so the *mechanism* is exercisable in simulation, but the silicon evidence does not compel it.
...
The Kunpeng-920 integrates the TaiShan V110 (TSV110) core: a 4-wide out-of-order ARMv8.2-A design with 64 KB 4-way L1D (256 sets, 64 B lines, **2×128-bit ports per cycle**), 512 KB private L2, and a 64 MB shared L3 partitioned per 4-core cluster. The LSU has 2 AGUs; store-to-load forwarding latency is 6–7 cycles (+1–2 across a 16 B boundary). *(Source caveat: these microarchitectural parameters are drawn from a community-maintained summary, `docs/cpu/kunpeng.md`, NOT a vendor datasheet — HiSilicon does not publish a public TSV110 RTL/datasheet. The set-association geometric adjudication in MICROARCH_SUPPLEMENT §2.1 (set 87 vs 105, which excludes the L1D array as the D1 source) is therefore **conditional on this geometry being correct**; if the vendor discloses different associativity or set-index bit slicing, the "D1 is fill-buffer not L1D-array" exclusion must be re-evaluated. We flag this as a conditional argument, not an established one.)* The vendor documents ECC on L1/L2 and "enterprise RAS" but does not disclose the coverage stage of the ECC checkers relative to the fill-buffer merge and output mux — a gap we return to in §6.
...
- **Boot 15:58 (hard evidence):** `x20 = 0x00ffffcc879da2e0` matches `ror1(__per_cpu_offset[0])` (right-rotate by one byte) at Hamming distance 0 (the nearest other candidate is slot[3] at distance 2 — see the reproducibility-grading note below; this "nearest" figure requires the full 192-slot array, which lives only in the non-redistributable 180 GB vmcore). The load targeted slot 146. **Independently re-verified from the original 0102-board vmcore**: `crash` reads `__per_cpu_offset[0] = 0xffffcc879da2e000` (@ `0xffffb378e29e55d0`) and `__per_cpu_offset[146] = 0xffffcc879ed92000` (the truth the load should have returned); `ror1(slot[0]) = 0x00ffffcc879da2e0 == x20` (Hamming 0), and `Hamming(x20, slot[146]) = 26` (confirming x20 is corrupted, not the truth). *Rotation-direction degeneracy (symmetric to the 08-14 case): for a 64-bit word, `ror1(x) ≡ rol7(x)` (1+7=8 bytes = whole word), so `rol7(slot[0])` *also* matches x20 at Hamming-0; the 15:58 signature is therefore identified under a (ror1, rol7) ambiguity, exactly parallel to the 08-14 (ror6, rol2) ambiguity noted below. The load-bearing fact is the Hamming-0 byte-rotation match itself — which is unreachable by any bit-flip — not the *right*-rotation direction (either of ror1/rol7 reaches it with 0 bit flips).* *(An earlier draft wrote `rol1` here; that was a notation slip — but the slip also masks the deeper point that the direction is genuinely ambiguous, which we now disclose.)*
...
Two independent boots, two different rotation magnitudes (1 byte for 15:58; 6-or-2 bytes for 08-14 per the degeneracy above), both hitting the *array head* (slot 0 in both boots — the 08-14 boot's `__per_cpu_offset[0]` is `0xffffd93715b7e000`, a different value from 15:58's `0xffffcc879da2e000` because the per-cpu offset array differs across boots/KASLR, but the matched slot is index 0, the head, in both) — 15:58 at Hamming-0, 08-14 at Hamming-6. We do **not** over-claim a "random coincidence probability" here (a prior version cited ≈2⁻⁵⁸; we withdraw it as circular — the fill-buffer stale-replay model *predicts* that the corrupted value comes from the oldest fill-buffer entry, which is the array head first-touched at boot, so "matching the head" is model-consistent, not a coincidence to be excluded). The load-bearing D1 evidence is instead: (i) the Hamming-0 byte-rotation match of the 15:58 case (reproducible bit-exactly; the nearest non-head candidate is at distance 2), and (ii) the **bit-flip unreachability, now stated with an explicit exhaustive bound**. *Labeling caveat (honest):* `slot[0]` is the **stale-replay source** (the value the load actually returned, per the stale-replay model), **not "the truth"** — the load targeted `slot[146]`, so the *truth* is `slot[146]=0xffffcc879ed92000`. We report both XOR-distances: from the stale-replay source `slot[0]` to the observed `x20`: `slot[0] ^ x20 = 0xff00334b1a3f42e0`, popcount **30**; and from the *truth* `slot[146]` to `x20`: `slot[146] ^ x20`, popcount **26**. The relevant bound depends on which value a bit-flip fault model would operate on: under the stale-replay premise the load returned `slot[0]`, so a bit-flip on the returned value would need *exactly 30 bits*; if instead one supposes the fault bit-flipped the truth `slot[146]` in transit, the bound is 26. Either way (k<30 or k<26) no pure bit-flip model reproduces the signature, whereas a single structural byte-permutation (ror1, ≡ rol7) reaches it with *0 bit flips*. We retain 30 as the headline bound (it is the conservative — larger — of the two, and corresponds to the paper's own stale-replay model), and disclose 26 for completeness so a skeptic can see both. An exhaustive search over the single-byte bit-flip space (8 bytes × 256 masks) finds no match (confirming single-byte, ≤8-bit flips are insufficient); the XOR-distances of 30 (stale source) and 26 (truth) close the bound for any k-bit pure-flip model with k < 30 (resp. k < 26). The corruption is thus a structural byte-lane re-route, not a bit flip — this is the signature of a fill-buffer/load-queue stale-entry replayed with a wrong byte-lane phase.

*Reproducibility grading (honest, referenced from the 15:58 hard-evidence note above).* The D1 load-bearing evidence is not all reproducible from the same artifact, so we grade it explicitly to avoid conflating "reproducible by anyone with the repo" with "reproducible only with the 0102-board vmcore":
- **(i) Repo-verbatim-reproducible** (from the cited values alone, no vmcore needed): `ror1(slot[0]=0xffffcc879da2e000)==x20=0x00ffffcc879da2e0` at Hamming-0; `ror1≡rol7` tie; `slot[0]^x20` popcount 30; `slot[146]=0xffffcc879ed92000` truth, `slot[146]^x20` popcount 26; exhaustive single-byte bit-flip on slot[0] yields no match. These four are the load-bearing D1 facts and any reader with Python can recompute them.
- **(ii) Vmcore-only** (requires the full 192-entry `__per_cpu_offset` array, which lives only in the 180 GB non-redistributable vmcore — §8): the *uniqueness* of the slot-0 Hamming-0 match, and the "nearest non-head candidate is slot[3] at distance 2" figure. These strengthen (i) but are not necessary for it — the Hamming-0 match + 30/26-bit bounds + exhaustive no-hit close the argument without uniqueness. A holder of the 0102 board can re-confirm (ii) via `reproduce_d1_forensic.sh` (§8); we do not claim (ii) is reproducible from the repo alone.

### 3.3 The address-path signature D2

In two crashes (08-14, 08-24) the architectural address has MSB ≠ 0 (`0xd9…`, `0x55…`) yet the kernel-printed `FAR` has MSB = 0 (`0x00…`). We must be honest that this D2 evidence is **weaker than a prior version claimed**: (1) **`FAR_EL1[63:60]` is UNKNOWN/RES0 for translation faults** (§2.2, ARMv8-A ARM DDI 0487 §D13.2.30) — only `FAR[51:0]` (and up to [55:0]) is architecturally guaranteed, so the high-nibble difference is not, by itself, architectural evidence. (2) **TBI1 (EL1/kernel top-byte stripping) is NOT statically enabled** — objdump of the 0102-board vmlinux `__cpu_setup` shows no `TCR_EL1.TBI1` (bit 38) immediate; `CONFIG_ARM64_TAGGED_ADDR_ABI=y` only dynamically enables TBI0 (EL0/user) per-process. *(Caveat: this is a static analysis of the `__cpu_setup` initialization path only; a runtime `mrs tcr_el1` read — via a kernel module or crash's per-cpu TCR — would be the closed-form adjudication, and was not performed. If a later runtime write to TCR_EL1 sets TBI1, this static finding would not catch it.)* **However, the TBI analysis is *inapplicable* to these two cases (a correction to a prior "partial recovery" framing):** TBI1 controls whether the MMU strips the top byte of a *valid* kernel VA during translation; but the 0814/0824 `arch_addr` high bytes 0xd9/0x55 are **not** valid kernel addresses — they are themselves the D1-corrupted value's non-canonical high bytes (canonical kernel addresses are 0xffff…). So the TBI1-off finding ("the kernel does not top-byte-strip *valid* kernel addresses") tells us nothing about whether the MSB-vs-FAR difference on these *already-corrupted* addresses is a defect vs. an artifact of the corrupted input. TBI neither rescues nor further impairs D2 here — the analysis is moot for the cases it was invoked on. (3) The "arch MSB ≠ FAR MSB" observation is therefore **confounded by D1** (the architectural address is itself the D1-corrupted value), independently of the TBI point. **Net: D2 is downgraded from "confirmed-weak" to "unproven candidate" (we no longer claim "partially recovered" — the TBI "partial recovery" was a non-sequitur for already-corrupted addresses) — the address-path mechanism is exercisable in simulation (§5.3), but the silicon evidence is confounded by D1 (the architectural address is itself corrupted), and TBI analysis is inapplicable to the two cases in question.** Resolving D2 fully would require a case where the architectural address is a true canonical kernel address yet FAR shows a different high byte — no such case exists in the 5 dumps.

### 3.4 The PTW signature D3

73 "spurious translation fault" warnings, all on valid linear-map addresses (72/73 map to static `Initmem` NUMA ranges; the one vmalloc outlier is immaterial). All have MSB = 0xff — the *address* reached the MMU correctly (no D2), but the walk transiently failed and retried. **ESR shape (architecturally parsed against the ARMv8-A ARM, DDI 0487):** all 73 are Data Aborts from the current EL (EC=0x25) with DFSC=0x04 (level-0 translation fault); 70/73 carry `ESR=0x96000044` and 3/73 carry `ESR=0x96000004`. The differing bit is ISS[6], which is the low bit of **ISS[7:6]=S1PTW** (the stage-1 page-table-walk flag — *not* "Overlay" as an internal diagnosis note had it; bit 6 is architecturally the S1PTW low bit for a Data Abort). For the 70/73 with S1PTW=1, the fault was taken **during a page-table walk** — which is *expected* for a PTW-readout defect (D3), not anomalous. For the 3/73 with S1PTW=0 (`0x96000004`), the fault was taken **with no walk in progress** — mechanistically *not* a PTW-readout failure (no walk was active), and possibly a different mechanism (e.g., a TLB-entry or TLB-invalidation-timing cause). We therefore report D3 as "70/73 walk-consistent + 3/73 walk-absent" rather than 73 uniform PTW-readout events; the 3 S1PTW=0 events do not fit the D3 hypothesis cleanly and are disclosed rather than smoothed over. **The 3/73 S1PTW=0 events are, mechanistically, the subset most plausibly attributable to the *legitimate* same-CPU TLB-invalidation-timing race that mainline commit 42f91093b043 (Will Deacon's spurious-translation-fault patch) targets — a fault taken with no walk in progress is exactly what a stale/invalidated TLB entry racing against a concurrent invalidation produces, not what a PTW-readout (walk-active) defect produces.** We therefore treat **70/73** (S1PTW=1, walk-active) as the clean D3 population and **3/73** (S1PTW=0, walk-absent) as a possibly-legal-race admixture not smoothed away; the D3 load-bearing count is **70**, not 73. (The S1PTW=1 majority is *consistent with* D3; we no longer treat any reserved-bit-set as unexplained — the earlier "bit6 should be RES0" reading was a misread of the ISS layout.) Three rebuttals to the "legitimate OOO-walk race" objection: (1) 72/73 target static, boot-time, never-freed mappings → no concurrent map activity satisfies the mainline race precondition; (2) 100% on CPU 179 → a *cross-CPU* race would distribute across cores — but this rebuttal does **not** exclude a *same-CPU* speculative-walk race (the kind Will Deacon's spurious-fault patch commit 42f91093b043 targets), which would also cluster on one core; we lean primarily on rebuttal (1) (static mappings) and acknowledge (2) is weaker than once framed; (3) events fire at arbitrary uptime (6 min to 146 h) with no correlating kernel activity. Net: D3 is *consistent with* a PTW-readout defect on the 70/73 walk-consistent events, but we cannot exclude per-core PTW/TLB *timing marginality* (a slow corner rather than a structural defect) for the population — resolving structural-defect vs. timing-marginal requires the vendor RTL/DFT (§6), as with D1/D2.
...
The existing CHAOSLSQFwd corrupts one byte of store-forwarded data via AND/OR/XOR — a bit-flip model. The D1 signature (byte-lane rotation) is **not expressible as a bit flip** (§3.2), so we add a structural axis: `structuralFault ∈ {none, byte_lane_skew, all_zero}` with `skewBytes` (1–7, 0=random). The `byte_lane_skew` mode right-rotates the delivered word by k bytes; `all_zero` delivers an empty-slot word. The hook remains `lsq_unit.cc:1498` (post-forward `memcpy`). *(Honest scope note: a third mode, `stale_line_replay` — delivering the *content* of the oldest fill-buffer entry to test the "stale source" half of the D1 model — is mentioned in the FI_DESIGN_SUPPLEMENT §3.1 `Param.String` enumeration string, but the source enum (`CHAOSLSQFwd.hh:53-57` `StructuralFault`) carries only `{None, ByteLaneSkew, AllZero}` — `stale_line_replay` has **no implementation and no source-level design trace** (no comment, no TODO, no enum entry; a grep of the build path returns zero hits). So "designed but not implemented" overstates it; the accurate statement is "a design intent was named in the supplement's param string, but it was never designed into the source nor implemented." The "stale source" identity is verified by §3.2 forensics, not by H5 simulation. H5 therefore exercises the "byte-lane rotation" half of the D1 model, not the "stale entry" half.)*
...
### 5.1 H5 (consistency-checked): structural byte-lane-skew reproduces the oops chain
...
Under higher probability the skewed pointer eventually slips past the check and is dereferenced, producing gem5's `panic: Page table fault when accessing virtual address 0xf0000000000044573` — the non-canonical address from a byte-rotated pointer. This is the **end-to-end reproduction of core 179's D1 chain**: load returns a byte-skewed value → used as a pointer → non-canonical VA → page fault → Oops. Reproducible across seeds. **H5 is consistency-checked (mechanically reproduced, not independently verified)** — the `byte_lane_skew` injector's rotation operation *is* the D1 signature's defining operation, so reproducing the oops chain closes the conjecture-verification loop (causal chain: byte rotation → non-canonical VA → page fault is mechanically sound) but does not independently establish that the defect *is* a rotation; that rests on §3.2 forensics alone. (See §5.2 epistemic-status caveat for why this is a closure/consistency check rather than an independent Popperian test.)

### 5.2 Why bit-flip injection is insufficient: a consistency/closure claim with an exhaustive bound (and a boundary test)

We proved (§3.2) that no pure bit-flip produces the observed corrupted value: an exhaustive single-byte search (8 bytes × 256 masks across all 192 array slots) finds no match, and the XOR-distance from the stale-replay source `slot[0]` (the value the load actually returned, per the stale-replay model) to the 15:58 signature is popcount-30 (and from the *truth* `slot[146]`, popcount-26) — i.e. a pure bit-flip model would need to flip *exactly 30 bits* of the returned value (or 26 of the truth) (a large, structured set, not a "few-bit" SEU) to reproduce it, whereas the structural `byte_lane_skew` mode reaches it with 0 bit-flips via a single byte permutation. We elevate this from a case observation to a **falsifiable methodological claim**, now stated with an explicit exhaustive bound:

> **For a defect whose signature is a *byte-lane phase displacement* of a stale value (Hamming-0 to a rotated copy of an array-head entry, whose XOR-distance from the stale-replay source is 30 bits / from the truth is 26 bits, not expressible as any single- or few-bit flip with k < 30 / k < 26), a bit-flip fault injector is in principle insufficient to reproduce the signature; a *structural* (byte-re-route) fault model is required.**
...
|---|---|---|---|---|
| `byte_lane_skew` (structural) | 30 | 28 | 28 | byte rotation (Hamming-0; ror1≡rol7 for 15:58, see §3.2) |
| `all_zero` (structural) | 30 | 29 | 29 | all-zero delivery |
| `bit_flip` (1-bit) | 30 (numBitFlips=30) | 29 | 29 | 1-bit mask (e.g. 0x20) |

**Honest finding:** the three fault modes produce *comparable detection rates* — `byte_lane_skew` 28/30 = 93%, `bit_flip` 29/30 = 97%, `all_zero` 29/30 = 97% — within n=30 statistical noise but not identical (the D1-relevant `byte_lane_skew` mode is 1/30 lower than `bit_flip`/`all_zero`). So "bit-flip insufficient" is **NOT about detection rate** — it is about **signature form**: bit-flip produces 1-bit-difference corruptions, not the Hamming-0 byte-rotation match that defines the D1 signature. *(Caveat on the boundary test itself: the ptrskew probe counts `PTR_CORRUPT = (loaded pointer ≠ golden)`, which fires for ANY value corruption, so the table's dependent variable is insensitive to signature form — the signature-form conclusion comes from §3.2's exhaustive value-matching, not from this table's detection counts. The table is illustrative of detection comparability, not probative of signature form.)* The §3.2 exhaustive test (no single-byte bit-flip on slot[0] yields the observed value) is what makes bit-flip *signature-incompatible* with D1, not detection-rate-incompatible. **This precisely scopes the §5.2 claim**: bit-flip is a valid model for many SDCs (and produces detectable corruption here), but it cannot reproduce the *byte-phase-displacement signature* that core 179 exhibited; a structural model is required for *that signature*, not for SDC detection in general.

**Epistemic-status caveat (honest).** We do **not** call this claim "falsifiable in the Popperian sense" — a Popperian test requires a falsification branch that is *reachable*, whereas here the falsification branch ("a bounded bit-flip reproduces a byte-phase-displaced signature") is unreachable by construction once the 30/26-bit XOR bound holds (bit-flip cannot reproduce the value) and the structural model is definitionally capable of byte rotation (it is the model built *from* the signature). The §5.2 claim is therefore a **consistency/closure argument**, not an independent Popperian test: it shows that the structural model *closes* the conjecture-verification loop by reproducing the causal chain, and that bit-flip is *signature-incompatible* — but a genuine independent falsification would require predicting a property (e.g., the rotation direction/magnitude or the stale-source identity) *before* measuring it on a new dump. We could not construct such a pre-registered prediction from the current single-case evidence. The claim's *scope* is honestly bounded — it applies to the byte-phase-displacement class of signatures, not to all SDC (many SDCs are legitimately single-bit SEUs, where bit-flip is the correct model, and the boundary test shows bit-flip produces comparable detection rates). But within its scope it is a toolkit-design implication: **structural data-path faults are a necessary addition to fault-injection toolkits for the byte-phase-displacement signature class**; bit-flip-only injectors (the CHAOS/GeFIN/SiliFuzz norm) cannot *reproduce this signature form*, and a "clean" bit-flip SDC study that omits structural faults will silently under-cover this signature class. This is the paper's transferable methodological contribution — independent of the specific defect it was derived from, and now boundary-tested against bit-flip.
...
ECC-on: 0 spurious across all 5 seeds (every single-bit flip corrected → valid PTE returned). ECC-off: 1–4 spurious per seed (the flip survives → invalid PTE → translation fault retried successfully on re-walk). **H7 is directionally consistent (ECC-on → 0 spurious 5/5, ECC-off → 1–4 spurious 5/5), but this is a near-tautological contrast, not a verification of ECC-correction behavior**: **the two arms are NOT strictly same-path controlled, and the ECC-on arm is a no-perturbation control rather than an ECC-correction demonstration** — the `conditionalValidBit` flip is gated out *before it happens* when `ptwEcc` is on (see `CHAOSPTW.cc` early-return at the ECC gate), so ECC-on's 0-spurious result reflects "no perturbation occurred" rather than "a flip occurred and was corrected in-place." ECC-off, by contrast, is perturbed and its surviving invalid PTE triggers a retry cascade that changes the execution path (`numHooksCalled` ≈ 15 808 vs ≈ 17 for ECC-on, `simInsts` ≈ 3 090). So the 5/5 directional consistency (ECC-on always 0, ECC-off always >0) is mechanically expected (inject nothing → observe nothing; inject a fault → observe spurious), and we do **not** call it "robust" evidence about ECC on silicon — it establishes only injector self-consistency (the conditionalValidBit flip, when allowed to land, produces spurious faults), not that ECC corrects a landed flip. A strict same-path contrast that would actually test ECC correction would require an in-place ECC-correction model (flip happens, is corrected, execution proceeds on the same path) or a non-cascading fault model — future work. (Data: `FI_DESIGN_SUPPLEMENT.md` §7, branch `fi-h6-h7-fs-verify` commit `3287299`; to be independently re-confirmed on a rebuilt `gem5.opt` once the user-space build chain is re-established on the current host — see §7.)

### 5.5 D2 vs D3 trigger density (a methodological finding)

The density table above is itself a result: the D2 (load-path) trigger base is ≈3 500× denser than the D3 (walk) base (61 081 vs 17 at 400 M). This means H6's D2 arm is *sample-feasible* in the early-boot budget in a way H7's D3 arm is not. It also explains, post hoc, why the on-silicon D3 symptom (73 spurious faults; 70 clean-D3 + 3 possibly-legal-race per §3.4) is rare relative to the D1/D2 load-path symptoms: on silicon too, the walk path is exercised far less often than the load path, so a walk-path defect surfaces as a low-rate spurious-fault stream rather than a high-rate SDC stream — exactly the 73-vs-5/78 split we observe.

---

## 6. Recommendations to the Silicon Vendor

1. **fill-buffer merge / byte-lane-mux at-speed scan.** The D1 signature — a 1-or-7 byte rotation (ror1≡rol7 ambiguity, §3.2) at Hamming-0 (15:58, bit-exact) and a 6-or-2 byte rotation (ror6≡rol2 ambiguity, §3.2) at Hamming-6 (08-14, nearest) — is a direct DFT vector for the fill-buffer byte-lane selection/merge logic; cover the load-return mux's 8 byte-lane phase controls (in particular the phase controls for both 1-and-7 and 2-and-6 byte displacements, since each ambiguity pair reaches the same signature) and reproduce the "cross-set stale-head replay" condition.
2. **AGU→MMU address-path byte7 path-delay test (conditional on D2).** *If* D2 is real (its silicon evidence is unproven — §3.3; TBI analysis is inapplicable to the 0814/0824 cases, which are confounded by D1), its MSB-zeroing would be a small-delay-fault fingerprint on the address-presentation latch byte7. We list it as a DFT vector *to test whether D2 exists*, not as evidence it does.
...
- **The fault host is the defect host.** gem5 was built and all FI runs executed with CPU 179 isolated via `taskset`; the link phase saw repeated transient param-file failures (a known SDC-affected-compile signature), resolved by single-threaded (`-j1`) and cautious `-j4` linking on healthy cores. Repeated relink attempts after source edits intermittently produced no binary despite `scons` reporting success — consistent with SDC-affected linking. H5 was reproduced (the oops-chain reproduction ran successfully) on the *first* clean full build; subsequent rebuilds of the modified tree are less reliable. H5 and the FS-mode confirmations should be re-confirmed on a second healthy machine. (Here "reproduced" means the simulation ran and produced the oops chain; the epistemic status of H5 as a consistency check rather than independent verification is discussed in §5.1/§5.2.)
...
- **H6/H7 status is exactly bounded, not over- or under-stated.** The SE-mode null results are statically root-caused to the ARM MMU translation model (§2.4, §5.3–5.4), not to injector logic. The FS-mode runs confirm D2 and D3 *hooks fire* under MMU-on translation and reproduce the on-silicon D2 signature (canonical→non-canonical). **H7 is directionally consistent (not "verified", not "robust")** (§5.4): the `conditionalValidBit` mode makes ECC the intended controlled variable, and 5 FS seeds (re-confirmed locally: ECC-on 0 spurious 5/5, ECC-off 1–4 spurious 5/5) show a directionally consistent contrast — but the ECC-on arm is a *no-perturbation control* (the flip is gated out before it happens), so the contrast is "no perturbation → 0 spurious" vs "perturbation → spurious", which is mechanically expected and establishes only injector self-consistency, not that ECC corrects a landed flip; a same-path in-place correction model would be needed for actual ECC verification. **H6 is direction-observed with within-FS multi-seed supporting evidence, but the observed separability is *stall*-separability, not SDC/Crash-separability**: the fetch-stall-as-Crash-proxy contrast is **separable across 5 seeds** (D1-only 5/5 normal progression vs D2-only 5/5 stall, D1+D2 5/5 stall-with-D2-dominating, §5.3) — but D1's "normal progression" in FS does not mean "no SDC" (there is no ptrskew probe in FS, so D1 could be silently corrupting), and the ~3k stall is a generic non-canonical-VA response (D3 high-prob also stalls ~3k), not D2-specific; so H6 establishes stall-separability, not SDC/Crash-separability. This is the strongest controlled within-FS contrast given the gem5 O3 fetch-stall model limitation. The guest-visible Crash/SDC spectrum (a guest Oops rather than simulator stall) remains future work — bounded **not by time** (O3 FS ~12万 inst/s) but by the O3 fetch-stall model (KVM CPU for real faults loses the O3-LSQ hooks). A reviewer asking "did you verify H6/H7?" gets "H7 directionally consistent only — the 5-seed ECC contrast is a no-perturbation-vs-perturbation control that shows injector self-consistency, not ECC correction on silicon (ECC-on arm not actually injected); H6 direction-observed with 5-seed within-FS *stall*-separability (not SDC/Crash-separability, and the stall is not D2-specific); guest-visible spectrum bounded by O3 fetch-stall injector-architecture limitation, not time."
...
- **D2 is unproven (downgraded from confirmed-weak); TBI investigation does not recover it (a correction to a prior "partial recovery" framing).** The §3.3 D2 argument is weakened, and the earlier "TBI analysis *partially recovers* D2" framing was a non-sequitur: (1) `FAR_EL1[63:60]` is UNKNOWN/RES0 for translation faults (§2.2, ARMv8-A ARM), so the high-nibble difference is not architectural evidence; (2) **TBI1 (which controls EL1/kernel-mode top-byte stripping) is NOT statically set** — objdump of the 0102-board vmlinux `__cpu_setup` shows no `TCR_EL1.TBI1` (bit 38) immediate; `CONFIG_ARM64_TAGGED_ADDR_ABI=y` only dynamically enables TBI0 (EL0/user) per-process, not TBI1 (EL1/kernel) at boot. *(Caveat: static analysis of `__cpu_setup` only; runtime `mrs tcr_el1` not done.)* **But TBI1 controls top-byte stripping of *valid* kernel VAs, and the 0814/0824 `arch_addr` high bytes 0xd9/0x55 are *not* valid kernel addresses — they are the D1-corrupted value's non-canonical high bytes (canonical = 0xffff…).** So TBI1-off ("the kernel does not strip *valid* kernel addresses") tells us nothing about these already-corrupted addresses; the TBI analysis is inapplicable to the two cases it was invoked on, and does not narrow D2's uncertainty either way. (3) The "arch MSB ≠ FAR MSB" observation is confounded by D1 (the architectural address is itself the D1-corrupted value), independently of TBI. D2 remains a *candidate hypothesis* (the address-path mechanism is exercisable in simulation, §5.3, but silicon evidence is confounded by D1). Resolving D2 fully would require a case where the architectural address is a true canonical kernel address (0xffff…) yet FAR shows a different high byte — no such case exists in the 5 dumps.
...
All vmcore-derived artifacts (`p1_events.csv`, per-cpu arrays, panic blocks), the three injector modules (`CHAOSLSQFwd`/`CHAOSAddrPath`/`CHAOSPTW`), the probe (`ptrskew_kernel.c`), the SE and FS experiment configurations (`o3_chaos_smoke.py`, `o3_chaos_fs.py`), the experiment scripts (`run_H6.sh`, `run_H7.sh`), and the full diagnosis reports are on branch `fix/paper-review-v1-honesty-hardening` (the honesty-hardening branch accumulating the REVIEW_v1/v2 fixes; the historical working branch was `docs/core179-microarch-rootcause`, and the FS H6/H7 verification code on `fi-h6-h7-fs-verify` commit `3287299`). **Reproducibility scope (honest):** `run_H6.sh`/`run_H7.sh` reproduce only the **SE-mode null results** (§5.3–5.4) — they run the SE config (`o3_chaos_smoke.py`), under which the D2/D3 hooks never fire. The **FS-mode 5-seed H6 and H7 tables** (§5.3 line 297–303, §5.4 line 345–351) were produced by **manual** invocation of `o3_chaos_fs.py` across seeds 0–4 × arms; **there is no single-script automated FS harness** for them (FS single-boot-to-bash is ≈1–2 h wall, and 5 seeds × arms is not feasible in one shot). The exact per-seed FS command lines are documented in `FI_DESIGN_SUPPLEMENT.md` §7; reproducing the FS tables requires ~10–20 h of FS runs on a verified `gem5.opt` build with the `gem5-fs/` four-file set. So the Honesty Preamble's "every quantitative claim is reproducible" should be read as: the arithmetic (XOR/Hamming/rotation) is reproducible from the cited values, the SE nulls are reproducible from `run_H6/H7.sh`, and the vmcore-derived forensics are reproducible by anyone with the 0102 board via `reproduce_d1_forensic.sh` — but the **FS 5-seed quantitative tables are not reproducible by a single shipped script**; they are documented manual runs. The vmcores themselves are 180 GB and not redistributable, but every claim cites a reproducible `crash`/`objdump`/`python`/`gem5.opt` command in the supplementary reports. The FS support files (`gem5-fs/`, ≈2.5 GB) are `.gitignore`d (only the README is tracked) but are described and path-verified in `gem5-fs/readme.md`.

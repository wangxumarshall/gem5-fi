...
1. **A three-path microarchitectural decomposition (D1/D2/D3)** of the defect, derived bit-exactly from five vmcores and the published TSV110 cache geometry, *hypothesizing* the SDC to the load data-return path (D1, **established**), the AGU→MMU address path (D2, **unproven candidate, partially recovered — see §3.3**), and the page-table-walker readout path (D3, **strong evidence**) — with pre-emptive rebuttals of the three strongest reviewer attacks (coincidence, register-dump staleness, legitimate OOO-walk race). The decomposition is a *hypothesis hierarchy*, not a uniform localization: D1 is bit-exact (Hamming-0 rotation + bit-flip unreachability, independently re-verified from the original 0102-board vmcore); D3 has 73 spurious-fault events on static mappings; D2's FAR-MSB evidence is confounded by D1 (the architectural address is itself the D1-corrupted value), though TBI1-off means software stripping does not explain it — so D2 stands as a candidate, not a finding.
...
- **Boot 15:58**: `x20 = 0x00ffffcc879da2e0` matches `rol1(__per_cpu_offset[0])` — *uniquely* (Hamming distance 0; the nearest other candidate is slot[3] at distance 2). The load targeted slot 146. **Independently re-verified from the original 0102-board vmcore**: `crash` reads `__per_cpu_offset[0] = 0xffffcc879da2e000` (@ `0xffffb378e29e55d0`) and `__per_cpu_offset[146] = 0xffffcc879ed92000` (the truth the load should have returned); `rol1(slot[0]) = 0x00ffffcc879da2e0 == x20` (Hamming 0), and `Hamming(x20, slot[146]) = 26` (confirming x20 is corrupted, not the truth).
...
A canonical kernel address (`0xffffffc0…`) had its byte7 zeroed to `0xffffc0…` (non-canonical) — the *simulation-side* D2 mechanism (canonical→non-canonical). Note this reproduces the D2 *mechanism* (byte7-zeroing yields non-canonical VA → fault), not the silicon D2 *evidence* (which §3.3 shows is unproven due to FAR[63:60] UNKNOWN + TBI). **SE mode cannot produce this** (the zeroed address still falls in physical memory and faults nowhere).
...
**What is established:** D2 FS injection reproduces the §3.3 signature (canonical→non-canonical), consistently derails execution (3/3 seeds + 16 B run), and D1 produces SDC-detectable corruption (SE 93%; FS fires 227× at 16 B without halting). **What is NOT established:** a *guest-visible* Crash/SDC classification. This is **not merely a time-budget issue** (an earlier draft cited ≈25 h; we measured O3 FS at ~12万 inst/s, so reaching bash is only ~5 min) — it is a **gem5 O3 model limitation**: when D2 injects a non-canonical address on a kernel load, gem5's O3 stalls fetch (`outside of physical memory, stopping fetch`, `simInsts` plateaus ~1.5 M after just 2 D2 injections at prob=1e-5) instead of raising a guest-visible translation fault → Oops. The CHAOS D2/D1 injectors hook the O3 LSQ (`lsq.cc`/`lsq_unit.cc`), so switching to KVM CPU (which would produce real translation faults) loses the hook points entirely. Thus a guest-visible H6 spectrum would require either (a) a non-O3 fault model that still hooks the address path, or (b) post-hoc analysis of the fetch-stall as a Crash proxy — both beyond this paper. H6 is **direction-observed with within-FS supporting evidence, not separability-confirmed**; the within-FS multi-seed separability and guest-visible spectrum are future work bounded by this injector-architecture limitation.

...
- **D2 is unproven (downgraded from confirmed-weak); TBI investigation strengthens the downgrade.** The §3.3 D2 argument is three-way weakened: (1) `FAR_EL1[63:60]` is UNKNOWN/RES0 for translation faults (§2.2, ARMv8-A ARM), so the high-nibble difference is not architectural evidence; (2) verified `untagged_addr(arch_addr) == FAR` in both 0814/0824, which is the *expected no-defect* behavior if `TCR_EL1.TBI0/TBI1` is enabled; (3) only 2/5 panics. The TBI setting was investigated on the 0102 board: kernel config has `CONFIG_ARM64_TAGGED_ADDR_ABI=y` (tagged-address ABI enabled — the user-space prerequisite for TBI), so the kernel *supports* TBI; we could not directly read `TCR_EL1` (crash 8.0.4 does not expose ARM system registers via `rd`, and `gdb info sysregs` did not return them in the crash-embedded gdb), but the tagged-ABI config + the verified `untagged_addr(arch_addr)==FAR` together make it most likely that the 0814/0824 FAR-MSB difference is normal top-byte stripping, not a hardware address-path fault. D2 stands as a candidate hypothesis, not a finding. (Also note: the 0814/0824 `arch_addr` high bytes 0xd9/0x55 are themselves the D1-corrupted value's non-canonical high bytes, not true kernel addresses — further weakening the D2 "address-path" reading.)

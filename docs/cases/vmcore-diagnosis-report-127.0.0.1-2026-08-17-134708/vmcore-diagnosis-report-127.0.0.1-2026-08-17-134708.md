...
> **补写身份声明**:本报告撰写于 2026-09-04,晚于第 3~6 次转储报告(08-24/08-25×2/08-26)成文;但所诊断的转储在时序上为第 2 次致命转储(开机时间全局编号)。报告中引用的"既往已证"结论来自第 6 次报告(08-26,`docs/cases/vmcore-diagnosis-report-127.0.0.1-2026-08-26-103727/`)与 core179 微架构根因综合研究,引用处均已标注。
...
- 26 条 WARNING 呈两簇结构:簇 1(47.0h 附近,15 条,密集,18:14~18:44)→ 17.8h 静默 → 簇 2(65.3~66.5h,11 条)→ 262.9s 后 panic。脉冲式(簇状)发作模式与第 8 案(09-03,35 条簇状爆发)的观察一致,支持"故障率随时间/条件窗口波动"的既有判断;
...
[239528.125975]  rebalance_domains+0x160/0x3b0
[239528.132091]  _nohz_idle_balance.isra.0+0x258/0x3c8
[239528.137668]  run_rebalance_domains+0x6c/0x88
[239528.120532]  handle_softirqs+0x128/0x330
...
...
- 非规范地址:FAR=`00ffd780f5a3a7c0` 高 16 位为 `00ff`,落在用户态与内核态地址区间之间(dmesg 原文 `address between user and kernel address ranges`,行 3769),FSC=L0(PGD 级翻译失败),是撕裂移位族的标准指纹(零塌缩族则是 `ffff…97e0` 型规范地址 + L3)【实锤】。
...
- 跨开机寄存器不变式本案全部复现【实锤】:x23≡`0x400`、x22==x26==`ffff604003e9ec00`(sched_group 成对指针)、x21−x24=`−0x5350`(`nr_cpu_ids` 与 percpu 页锚的相对距离,`algebra.py` [5] 复算 `0xffffffffffffacb0` 即 −0x5350)。
...
- 崩溃进程是 `swapper/179`(PID 0):CPU179 的 idle 线程。它不承载任何用户业务,崩溃发生在 `do_idle → cpuidle_idle_call → default_idle_call` 中断返回后的 softirq 负载均衡里。这次踩坏崩溃的不是业务进程,而是 CPU 空闲时的内核后台调度路径:机器在无重负载、甚至局部空闲的状态下死亡。
- 对上层服务的表现:整机重启。idle 线程在 CPU179 上 panic 意味着内核无法继续调度(Oops 于 PID 0 + `SMP: stopping secondary CPUs`),全部 192 核上的业务(sftp 传输、mi-scavenger 扫描、RDMA 存储、KVM 等)随 kdump 转储后整体中断。本开机从 2026-08-14 19:14:59 到 2026-08-17 13:47:06 连续运行 66.5 小时,是已诊断各案中最长的一次存活(第 1 案 31.7h、第 3 案 149.3h 是 idle 外路径、第 6 案 18.5h)。66.5h 的业务连续性在毫无预兆告警(RAS 层零记录)的情况下被单核缺陷终止。
...
结论:vmcore-incomplete 缺失的页覆盖 percpu/运行队列等核心结构,`__per_cpu_offset` 数组真值、`vtop` 页表走查、反事实地址验证全部不可执行。本案内存维度的证据为零,以下归类据此降级。
...
x20 的 `00 ff` 高 2 字节是"64 位值整体右移一字节(≫8)"的确定签名:任何 `0xffff....` 形态的真值右移 8 位,高字节必为 `0x00`、次高字节必为 `0xff`。第 4 案(08-25 15:58)x20=`00ffffcc879da2e0` 曾在完整 vmcore 上被证实为 `__per_cpu_offset[0] ≫ 8`〔既往已证〕,本案 x20 与其高 3 字节逐字节相同(00 ff ff),形态同构。据此归入撕裂移位族·跨字节右移子族,置信度【强推】:本案无内存真值对照(P3 边界),若 vmcore 可载(数组真值与 x20<<8 的关系可验)则可升级为实锤。
...
- 撕裂移位族归类:【强推】。形态签名(高 2 字节 `00 ff`)与第 4 案已证子族逐字节同构,多源收敛;但本案 vmcore-incomplete,无内存真值对照,无法完成"内存完好 + 寄存器收坏"的决定性实验。若 vmcore 可载可升级为实锤(验证途径:`rd -64 __per_cpu_offset 192` 检查数组是否等差完好,并比对 `x20 << 8` 与真值关系);

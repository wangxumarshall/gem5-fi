...
- [ ] 1.1 复核勘察表数据(uptime/寄存器/前兆 8 次/末次前兆 9978.33 距致命 0.72s——**谱系最短前兆-致命间隔,须精确计算**)
- [ ] 1.2 python3 代数闭合(x27=x1+x20;FAR=x27+0x120)脚本写入案目录并运行
- [ ] 1.3 dmesg_forensics.txt(关键行原文+行号)、报告 md 撰写
- [ ] 1.4 验证(报告引用 vs dmesg 原文逐条对照)→ commit → push

## Task 2: 案 21(09-08-17:21:33)同上四步

- [ ] 2.1 复核(前兆 3 次;x20=d3eb5026e000ffff 高段塌缩形态分析——低 16 位 0xffff 标签与案 22-24-26-27 的窗口尾部 0xff/0xffff 系列归并)
- [ ] 2.2 代数闭合脚本+运行
- [ ] 2.3 附件+报告
- [ ] 2.4 验证 → commit → push
...
- [ ] 3.1 复核(**x25=179 自指装载**——CPU179 读自己的槽;零前兆;4 连崩第 1 案)
- [ ] 3.2 代数闭合
- [ ] 3.3 附件+报告
- [ ] 3.4 验证 → commit → push
...
- [ ] 4.1 复核(x20=ffa9342267e000ff vs FAR 低段 09aeec68998df 错位窗口几何)
- [ ] 4.2 代数闭合
- [ ] 4.3 附件+报告
- [ ] 4.4 验证 → commit → push
...
- [ ] 5.1 复核(受害进程 kworker/179:1H 同族;开机仅 1592s 即崩——谱系最短 uptime 崩溃之一;x12=0101010101010101 有趣但与崩溃无关)
- [ ] 5.2 代数闭合
- [ ] 5.3 附件+报告
- [ ] 5.4 验证 → commit → push
...
- [ ] 6.1 复核(ESR=0x96000006/L2 变体;零前兆零塌缩)
- [ ] 6.2 代数闭合
- [ ] 6.3 附件+报告
- [ ] 6.4 验证 → commit → push
...
- [ ] 7.1 复核(前兆 63 次;末次 21771.26 距致命 1.75s;x20 尾 0xff 窗口)
- [ ] 7.2 代数闭合
- [ ] 7.3 附件+报告
- [ ] 7.4 验证 → commit → push

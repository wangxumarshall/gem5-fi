...
- [x] 10.1 9 案增量小结写入本计划文件尾(或 SYNTHESIS 附记):4 连崩跨重启复现、前兆-致命间隔 0.72s 新纪录、184 前兆新纪录、neon_rot_ldr_at 首次成为受害进程、`__per_cpu_offset[7]` 真值对照(案 28)
- [x] 10.2 分支最终状态确认 + push


## Task 10 附记:9 案(案 20~28)增量小结(2026-09-15 完成)

**全部 9 案报告已提交**(docs/cases-2/vmcore-diagnosis-report-127.0.0.1-{09-07-200426, 09-08-172133, 09-11-143132, 09-11-163010, 09-11-170607, 09-11-175836, 09-12-000919, 09-12-100031, 09-14-014556}/),各含报告 md + algebra.py + algebra_out.txt + dmesg_forensics.txt(案 28 另含 crash_session.log 3178 行)。

### 谱系级新事实(对 SYNTHESIS-12case 的增量)

1. **09-11 四连崩 = 4 次独立冷启动**(案 22/23/24/25,uptime 15980/6648/1592/2777s 各异,journald 脏日志链互证)——故障跨重启稳定复现,排除易失性状态;连续 6 个 boot(至案 27)持续显形。
2. **前兆-致命间隔新纪录**:案 20 的 0.72s(旧纪录 17.6s 的 1/27.3);案 26 的 1.752s、案 27 的 15.9s、案 28 的 102.5s 共同支持"前兆与致命同一发作窗内先后被击中"模型。
3. **184 次前兆新纪录**(案 28,旧纪录 35 次的 5.3 倍):43 簇多发作窗结构(首窗 7.15h→25.02h 大静默→再发作→临终加密,末簇 35 次/250s)——谱系最精细前兆时间画像;案 26 三段式、案 27 临终双簇为其变体。
4. **形态签名跨 boot 复现**:案 26 与案 23 的"双端 ff 沾染"(ff??..00ff)跨 boot 同构——复现粒度从"族"细化到"签名";案 28 与案 20 同槽(x25=7)同 FSC(L3)跨 boot 复现。
5. **FAR 顶字节截断获第 2 例**(案 27 XOR=0xd700..与案 22 的 0x9200..同模式:仅顶字节清零,低 48 位一致)——从孤例假设升级为系统性痕迹。
6. **错排位置谱完整化**(lane 错位族):完整 ffff 落 [31:16](案22)、双端抛掷跨度 56 位(案23)、相邻字节撕裂 00ff|ffb8(案24)、相邻撕裂下移 16 位 00ff|ffab(案27)——同一标签对在 64 位窗口内任意 16 位边界组合均观测到。
7. **零塌缩几何镜像**(案 25):x20=0→FAR canonical 但 pmd=0(L2 断链)与 L0 案(noncanonical)互为镜像——同一腐化点,形态决定 fault 几何;谱系零塌缩族扩至 9 案(原 6+案20+案25+案28)。
8. **案 28 三重 crash 实测**(9 案唯一完整 vmcore):__per_cpu_offset[7] 真值 0xffffb5297a60c000 vs 实收 0(64 位全异);x17 残留槽 179 真值化石(同核近期装载正常的活证);反事实 runqueues[7]/[179] 两级换算逐位闭合(x1=&runqueues percpu 模板基址,后续 dmesg-only 案的 x1 解读锚点)。
9. **崩溃链谱扩展**:exit/do_task_dead 退出路径(案 27)与 idle softirq 路径(案 20/28)首见——调度器进入 __schedule 的睡眠/空闲/死亡三类路径全覆盖。
10. **进程身份澄清**:neon_rot_ldr_at 为 opendcdiag 派生每核满载探针(案 27 报告修正勘察表"本仓库探针"误记),首次成为致命受害者;与案 11(同进程为背景)合并排除压测进程特殊地位。
...
|---|---|---|
| 零塌缩 | 20, 25, 28 | 3 |
| lane 错位(高段注入/标签迁移/双端/撕裂) | 21, 22, 23, 24, 26, 27 | 6 |
(案 21 形态=lane 错位×高段塌缩混合,计入 lane 错位)

### 处置建议(维持并加重)

offline CPU179 后前兆/崩溃应归零(唯一根治手段);裸机 load-verify 双核校验;厂商 shmoo/MBIST 复测 RF 与 bypass(重点:字节粒度选通路径 + 零源选通);RAS 监测在位但零捕获——固件级错误注入验证 ghes_edac 覆盖面。

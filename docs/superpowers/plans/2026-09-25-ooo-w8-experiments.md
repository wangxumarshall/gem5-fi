...
12. **W8.2+ hang_timeout=300s 全局政策（用户批准 2026-09-25）**：选项 (b) 获批——W8.2 及以后所有量产 campaign 的 hang_timeout 统一 600→300（负载正常完成 ~57s，300s=5.3× 余量；停摆型结局分类=Hang 与超时值无关，判据只更严不松）。**Wave B 门跑批维持 600s 均匀口径跑完**（门数据一致性优先，改门内参数=方法学漂移）。落地方式：Wave B 结束后的 W8.2 备产单元（campaign.py 公式补丁为独立先行单元）里把 w82-w85 全部 stanza 的 hang_timeout 改 300 并解除 LAUNCH GUARD。W8.2 规模按 M3-gate 机制：M2 过门后先跑 W8.2（Int Rename 47 子 campaign）作吞吐验证批，报告实测预算后再续 W8.3-W8.5。
13. **M2 门语义重构（用户指令 2026-09-25，覆盖 06 §5 M2 与 04 D25/D28 行的旧框架）**：旧表述「须复现 TC'23 SDC≈0%；SDC>0=注入器有错、回修、全部作废」**作废**。本研究的目的是**打破/超越现有发现**，不是被它们定义——若把已发表数字当及格线，TC'23 结论就不可证伪，任何真反例都会被我们亲手当 bug 杀掉。新语义：
    - **注入器正确性 ≠ 结果与文献一致**。正确性判据=正向控制：注入证据日志（old/new/bits 可复算、落点正确）、同 seed 复现、L2 trace 链路、golden 无注入回归——这些全部已过（W4-W7.4 各批次三件套）。TC'23 数字是**基线参照**，不是 pass/fail 门。
    - **与 TC'23 的偏离=平台发现，逐条记录并报告**，已有两例：D25（ROB PC 字段）100% Masked vs TC'23 Crash 主导——gem5 O3 的 PC 字段是 commit 路径不消费的元数据（W5 D32/33 结构掩蔽的平台重证）；D28（dest-id）100% Hang vs TC'23 快速陷阱拦截——gem5 PRF 模型无依赖检查陷阱，真依赖指向错 tag 后永久停摆。两者均为「注入器已验证正确前提下的结果分布差异」。
    - **SDC 概念按传播轴理解（用户口径）**：掩蔽（未逃逸）/ 隐蔽传播（silent）/ 短暂传播后被保护机制捕获（panic/复位）——后两者都是传播，silent-vs-caught 是检测覆盖率差异而非「是否 SDC」的差异。五分类（Masked/SDC/Crash/Hang/Inactive）保持为**测量层**（粒度细，可后聚合）；**传播轴（propagated% = SDC+Crash+Hang 中真正传播的子集）为 W9 元分析的聚合层**，W9.2 报告双轴。
    - **formal 期间 SDC>0 的处置**：不是杀跑批，而是**调查协议**——①核注入证据日志（落点/位/值可复算？）②复核分类器（checksum 提取/oracle 对？）③若注入器无懈可击，则 SDC 是**结果**，如实回填并在批次小结中作为发现突出。Wave B 已在跑的 600s 数据不受影响（测量本身与框架无关）。

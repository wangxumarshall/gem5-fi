...
- [ ] **Step 4: 结构与配对检查**（同 Task 1 Step 4）
...
- [ ] **Step 1: 行号验证第六章引用**（tools/*.py 行号抽查，python 无 LSP 也用 sed 核对）

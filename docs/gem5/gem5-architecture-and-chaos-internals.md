...
先补一句背景：乱序机执行时，一条指令的目的寄存器并不直接写在架构寄存器上，而是由 Rename 阶段分配一个空闲的物理寄存器，把"架构寄存器 → 物理寄存器"的映射记在 RAT（Rename Alias Table）里。指令被 squash 时，还要能按映射历史把 RAT 退回去。理解了这个机制，下面的两张表就好读了。Rename 阶段维护**两张映射表**（`cpu.hh:459-463`）：

...
> 本章以 `figures/sdc-fig-arm64.html` 的 **A–H 八分区逻辑图**为纵轴（第 1、2 列），逐单元对齐
> Kunpeng 920（TSV110）、920f（part 0xd22）、Neoverse N1/N2/N3 五款核的准确配置（第 3–7 列），
> 最后按「现状 → 薄弱点 → 加固点」三层挖掘每款核的 SDC 敏感性。
>
> **记号约定**：`—` = 该核无此结构；`未披露` = 公开文档未给出；`未测` = 仅靠黑盒实测、无公开规格；
> `★` = 独有/标志性设计。单元格为精炼速记，详细展开见 §2.3。

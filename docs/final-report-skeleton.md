...
**v1.3 formal 级更新（2026-09-08,Phase 18/19,n=384 全零 frozen）**：
| rank | 结构 | P_SDC（formal） | 保护建议（更新） |
|---|---|---|---|
| 1 | L1D 数据阵列 | 97.7% | 数据级 ECC 有效 |
| 2 | L1D post-check 通路 | 90.9% | 通路级 parity（ECC 不覆盖） |
| 3 | FPU 数据通路（mant_hi on svd） | **92.4–100%（C0/C2 双平台）** | **数值级 ECC/位段（mant_hi 优先）——FP 通路平台稳定,纯 SDC 风险** |
| 4 | LSQ 转发源选择 | 37.6% | age/ID 校验 |
| 5 | **INT 执行通路（elemwise C0）** | **69.5% SDC + 20.1% DUE** | **整数通路指针校验（v1.2 新增行）——C2 上全 DUE 化,平台敏感** |
| 6 | **L2 tag（F5 合法别名 × secded）** | **51.2%——ECC 盲区** | **别名检测/双 tag 比较（非 ECC）** |
| 7 | DRAM 数据（定向 × secded+逻辑故障） | 83.3% | **ECC 逻辑自检（syndrome 全零校验）——数据面 ECC 的必要配套** |
| 8 | PRF X3 bit0（rob≤128） | 100% | 低位段 parity |

**第四条横断定律（v1.3 新增）——平台效应结构**：FP 数据通路跨平台稳定（错浮点恒合法→SDC 主导）；INT 数据通路平台敏感（C0 SDC 69.5% → C2 全 DUE 化,调度激进度决定错值多快变非法指针）；DRAM 层平台敏感（85.4%→30.2%,机理 open）。**保护设计必须按平台调度特性分层,不能只按结构**。
...
### §23 系统级互连（NoC/CHI/HCCS）——scope-cut 决定

**未测，按 scope-cut 处理**：V110 互连含链路级 CRC + 重传，且现场无互连故障信号——其 SDC 贡献低是**保护机制的结构性质**，与测量无关。其主导故障形态（合法但错位）已由三个测量点定界：**LSQ 错源转发 37.6% [32.8,42.6] / DRAM addr_map_sub 88.0% [80.2,93.0] / L2 tag 合法别名 51.2% [46.1,56.2]（ECC 盲区）**——合法域替换的 SDC 风险区间 [37.6%, 88.0%] 由这三个 formal/pilot 级数据点覆盖，读者按互连具体保护强度在该区间内自行定位。gem5 classic SE 无链路级结构可注入（做窄版=行为代理套行为代理，E3² 无可迁移含义——v1.1 FPU 0% 伪影的教训：注入点不在真实路径上的数字是假的）。**留作实机校准项。**

**§20 PTW/PTE H7 定稿(v1.3 Phase 20 formal)**:2-bit 相邻损坏 × 内核态 walk(TTBR1)× ECC {off,on}——**ECC off 49.0% [44.0,53.9] 致死(n=384:kernel panic 100 + fault 诱导 abort 88)vs ECC on 0.0% [0.0,3.7](n=100),CI 零重叠**。定界链:单 bit 用户态 PTE 错=内核重填自愈(0/30);2-bit 内核态=PTE ECC 是唯一防线。**PTE 行保护定稿:ECC 本体(防 2-bit 内核态)+ ECC 逻辑自检(防 §14 ecc_logic_fault 85% 击穿)缺一不可**。

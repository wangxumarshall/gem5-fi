...
**以两论文 14 个实验为基线的系统对比结论**：

1. **论文核心主张全部复现且无违反**：P2-E1 的 ACE 上界性质、
   E5 advice>盲变异、E7 coverage⇒detection（Round 2 强化至 24 倍）、
   截断/种子敏感性——仓库数据与论文方向一致且多处更强
   （ARM64 无 RAX quirk 方差≈0、1 条指令即饱和）。
2. **ED（本仓库扩展）在正面对比中落败**（CE-1）：论文的结构内
   ACE 标量在此池上是更好的预测器（AUC 0.60-0.67 vs ED 0.51-0.55）。
   ED 的四个度量层缺陷全部实证登记：wrapper 底噪、max 映射钝化、
   **足迹盲区（conflict_seq ED 中游但检测率全场最高）**、evolve
   协议分母耦合。
3. **ED 的组级/轴级价值保留**（Round 2 J1/J3）：作为序列生成定向
   目标（gap 配额）有效，作为连续排序度量需先修四个缺陷
   （Round 3 清单）。
4. **池多样性是下一个瓶颈**（CE-3）：臂间高相关说明需要真正
   多样化的序列（不同 mix/足迹/值域），而非同模板变体。
5. **不可比项诚实清单**：DIFT oracle、FPGA、µop 注入点、
   RISCV 对照（架构/环境差异，CE-4/5 声明）。

**对论文方法 vs 本方法的一句话裁决**：论文的"结构内覆盖标量 +
advice 进化"是经本仓库独立数据验证的稳健基线；ED 的贡献在
配置驱动的定向生成与跨 CPU 权重（J1/J3 实证），其聚合评分层
需要 Round 3 修复才能在连续预测上超过论文基线。

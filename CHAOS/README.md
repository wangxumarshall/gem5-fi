...
CHAOS is organized into 4 different modules, which offer the ability to inject faults into:
- CPU architectural registers (CHAOSReg).
- CPU physical register file cells (CHAOSPhysReg) — O3CPU only.
- Cache lines (CHAOSCache).
- Main memory locations (CHAOSMem).

All ISAs (ARM, NULL, MIPS, POWER, RISCV, SPARC, X86) and CPU models (O3CPU, TimingSimpleCPU, MinorCPU, AtomicSimpleCPU, DerivO3CPU, SimpleCPU) supported by gem5 are fully compatible with CHAOS — except CHAOSPhysReg, which is O3CPU-only (it reaches the physical register file, rename maps, and free list of the out-of-order back end).
...
## Usage of CHAOSPhysReg

CHAOSPhysReg is an O3CPU-only physical-register-file fault injector. Unlike CHAOSReg (which targets architectural registers via the rename/commit map), it targets **physical register cells directly**, matching the abstraction used by ITC'23 / GeFIN-style tools: a real defective cell does not know which architectural register currently maps to it. It requires three small accessors/hooks in the O3 sources (`cpu.hh`, `free_list.hh`, `regfile.hh`); these ship pre-patched in the vendored gem5 tree (see `BASE_VERSION`).

The following parameters are configurable:
- *cpu*: The target CPU (must be an O3CPU / DerivO3CPU). A non-O3 CPU causes a `dynamic_cast` failure at construction time.
- *injectionMode*: Selects the injection abstraction:
    - `'phys'` — inject by **physical** register index, regardless of the current arch mapping (the ITC'23/GeFIN abstraction; default).
    - `'arch_frontend'` — `renameMap.lookup(archReg)` → `setReg` (targets the phys reg in-flight instructions will read; the corrected arch abstraction).
    - `'arch_commit'` — `commitRenameMap.lookup(archReg)` → `setReg` (the original CHAOSReg behavior, which **fails on O3** because the committed mapping lags in-flight reads; kept for comparison only).
- *targetPhysRegIdx*: Physical register index (phys mode). -1 = random across `[0, numIntPhysRegs-1]`. The injector liveness-probes the free list and skips free (dead) slots unless targeting AVF of inactive cells.
- *targetArchRegIdx*: Architectural register index (arch_frontend / arch_commit). On AArch64, X0–X30 = 0–30 (index 31 = Zero is excluded).
- *probability*: Per-interval injection probability. Use `1.0` with `maxFaults=1` so a single injection lands at `firstClock`.
- *faultType*: `'bit_flip'` | `'stuck_at_zero'` | `'stuck_at_one'` | `'random'`.
- *faultMask*: 32-bit bitmask; 0 = randomly generated (`bitsToChange` bits).
- *bitsToChange*: Number of bits to affect when `faultMask` is 0.
- *firstClock*: First clock cycle eligible for injection.
- *lastClock*: Last eligible cycle. **0 = unrestricted.** Do not use small nonzero values as a window — they cause silent zero-injection (see CHAOSReg's `lastClock` trap). Use `maxFaults` for count control.
- *maxFaults*: Max faults to inject; 0 = unlimited. Use 1 for single-injection, attributable campaigns.
- *rngSeed*: RNG seed (`std::mt19937`). 0 = `std::random_device` (non-reproducible); nonzero = fixed (reproducible register/mask selection).
- *writeLog*: Write a `fault_injections.log` file.

After the run, `fault_injections.log` records each injected fault (cycle, target phys/arch index, mask, fault type, and liveness/overwrite diagnostics), and `stats.txt` reports:
- *system.CHAOSPhysReg.numFaultsInjected*: Total faults injected.
- *system.CHAOSPhysReg.numBitFlips* / *numStuckAtZero* / *numStuckAtOne* / *numPermanentFaults*: per-type counts.

CHAOSPhysReg additionally traces **reads of the injected value** (stops counting once the target phys slot is written and the injected value is destroyed) — the `reads_before_overwrite` diagnostic in the log, which closes the "did the fault actually propagate?" question that architectural injectors cannot answer.

# Campaign `w23-toy-smoke-prf-mb` — summary

- injector: `physreg`  config: `C3`  mode: `SE`
- cells: 1  reps done: 3  wall: 49s
- workload: `workloads/ooo/smoke/smoke`  golden_id: `smoke-golden-v1`
- base_seed: 20260924  (rep seed = base + cell_ordinal*1000 + rep)
- trace two-pass (W2.3): ref_run=`once` replay_for=`['SDC', 'Crash', 'Hang', 'Masked']` replayed reps: 3 (five-class commit_diff result + latency in each replayed rep's `l2` block in results.jsonl; ref traces at runs/<cid>/ctrace_ref_cNNNN.csv.gz)

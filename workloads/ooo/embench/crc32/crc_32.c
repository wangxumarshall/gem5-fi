...
verify_benchmark (int r)
{
  /* gem5-fi W1.2: FINAL=<16hex> oracle line for tools/classify.py
     _CHECKSUM_RE.  FNV-1a 64-bit hash over the verified CRC result r
     (the benchmark's own checksum output, matching the "crc32 already
     prints a deterministic check value" case in the W1.2 plan).  Printed
     on the verified-success path only; a verify failure still makes
     main return 1 (-> Crash/DUE in classify.py).  Benchmark logic is
     untouched.  NB: r is the upstream result fold (r % 32768), so this
     oracle carries the same 15-bit entropy as upstream's own check --
     any CRC corruption that upstream catches is a verify failure, not
     an SDC. */
  if (11433 == r)
    {

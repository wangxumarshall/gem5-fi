...
verify_benchmark (int r)
{
  ...
  /* gem5-fi W1.2: FINAL=<16hex> oracle line for tools/classify.py
     _CHECKSUM_RE.  FNV-1a 64-bit hash over the full 4-word MD5 digest
     {h0,h1,h2,h3} (file-scope state left by the last md5() call) --
     strictly stronger than the upstream XOR fold r = h0^h1^h2^h3, which
     can mask a two-word corruption.  Printed on the verified-success
     path only; a verify failure still makes main return 1 (-> Crash/DUE
     in classify.py).  Benchmark logic is untouched. */
  if (r == RESULT)
    {
      const uint32_t digest[4] = { h0, h1, h2, h3 };
      const unsigned char *p = (const unsigned char *) digest;
      uint64_t h = 0xcbf29ce484222325ULL;
      unsigned fi;
      for (fi = 0; fi < sizeof digest; fi++)
	{
	  h ^= p[fi];
	  h *= 0x100000001b3ULL;
	}

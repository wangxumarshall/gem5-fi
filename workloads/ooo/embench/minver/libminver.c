...
#include <math.h>
#include <string.h>
#include <stdio.h>		/* gem5-fi W1.2 */
#include <stdint.h>		/* gem5-fi W1.2 */
#include "support.h"
...
verify_benchmark (int res __attribute ((unused)))
{
  ...
  /* gem5-fi W1.2: FINAL=<16hex> oracle line for tools/classify.py
     _CHECKSUM_RE.  FNV-1a 64-bit hash over c, d and det -- the same
     3x3 float matrices and determinant the loop above checks.  Printed
     on the verified-success path only; a verify failure still makes
     main return 1 (-> Crash/DUE in classify.py).  Benchmark logic is
     untouched.  Byte-level FP oracle, strictly stronger than the
     upstream epsilon compare (a within-epsilon drift that upstream
     tolerates still changes the hash); gem5 SE fplib has proven
     bit-exact vs hardware on this platform (workloads/ooo W1.5b/W1.5c),
     so native == gem5 at the bit level (verified in the W1.2 record). */
  if (float_eq_beebs(det, -16.6666718))
    {
      uint64_t h = 0xcbf29ce484222325ULL;
      const unsigned char *pc = (const unsigned char *) c;
      const unsigned char *pd = (const unsigned char *) d;
      const unsigned char *pt = (const unsigned char *) &det;
      unsigned long fi;
      for (fi = 0; fi < sizeof c; fi++)
	{
	  h ^= pc[fi];
	  h *= 0x100000001b3ULL;
	}
      for (fi = 0; fi < sizeof d; fi++)
	{
	  h ^= pd[fi];
	  h *= 0x100000001b3ULL;
	}
      for (fi = 0; fi < sizeof det; fi++)
	{
	  h ^= pt[fi];
	  h *= 0x100000001b3ULL;
	}

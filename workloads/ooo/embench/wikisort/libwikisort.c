...
verify_benchmark (int res __attribute ((unused)))
{
  ...
  /* gem5-fi W1.2: FINAL=<16hex> oracle line for tools/classify.py
     _CHECKSUM_RE.  FNV-1a 64-bit hash over array1, the same sorted
     400-element Test array the memcmp above checks.  Printed on the
     verified-success path only; a verify failure still makes main
     return 1 (-> Crash/DUE in classify.py).  Benchmark logic is
     untouched. */
  if (0 == memcmp (array1, exp, max_size * sizeof (array1[0])))
    {
      const unsigned char *p = (const unsigned char *) array1;
      uint64_t h = 0xcbf29ce484222325ULL;
      unsigned long fi, n = max_size * sizeof (array1[0]);
      for (fi = 0; fi < n; fi++)
	{
	  h ^= p[fi];
	  h *= 0x100000001b3ULL;

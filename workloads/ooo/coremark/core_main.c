...
#error "Please define a way to initialize a memory block."
    ...
    /* gem5-fi W1.1: FINAL line for oracle chain. Printed on the CRC
       checksum validation passed path (known seed set + crclist,
       crcmatrix and crcstate all matched => results[0].err == 0), not on
       total_errors == 0: the upstream "at least 10 secs" score-publication
       rule increments total_errors under gem5 SE (clock_gettime returns
       simulated time) and would suppress the line. Value is CoreMark's
       computed final CRC (crcfinal), zero-padded to 16 hex digits to match
       tools/classify.py _CHECKSUM_RE. */
    if ((known_id >= 0) && (results[0].err == 0))
        ee_printf("FINAL=%016x\n", (ee_u32)results[0].crc);
